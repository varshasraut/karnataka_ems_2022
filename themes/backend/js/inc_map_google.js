...
//    }
   ...
   var myLatLng= {lat: parseFloat(lt), lng: parseFloat(lg) };
   var $geocoder = new google.maps.Geocoder;
    $geocoder.geocode({'location': myLatLng}, function(results, status) {
        if (status === 'OK') {
            
                document.getElementById("inc_map_address").value = results[0].formatted_address;
                //document.getElementById("inc_map_address").focus();
                $('#inc_map_address').prop('disabled', false);
                $('#search_amb').show(0);
                
                lbs_place_changed(results[0]);
        }
    });
    
//        $.ajax({
//            url: 'https://maps.googleapis.com/maps/api/geocode/json?latlng='+latlng+'&key=AIzaSyCVgFhdOAhLy-jj7lDyUDFh0QRwIe6gYo4',
//            type: 'GET',
//            dataType: 'text',
//            jsonp: 'jsoncallback',
//            success: function (data) {
...
//                document.getElementById("inc_map_address").value = data.results[0].formatted_address;
//                //document.getElementById("inc_map_address").focus();
//                //$('#inc_map_address').prop('disabled', false);
//                lbs_place_changed(data.results[0]);
//            }
//
//            }
//          });
    ...
    $("#inc_ext_map_address").autocomplete({
       ...
       // console.log(  ui.item );
         ...
         source: function (request, response) {
             ...
             $.ajax({
               // url: "https://places.ls.hereapi.com/places/v1/autosuggest?in=34.083813%2C74.809463%3Br%3D5000000&size=1000&tf=plain&addressFilter=stateCode%3DMH&X-Mobility-Mode=drive&Accept-Language=en&app_id=hSm0zJfek39BQxeXGRYZ&app_code=46aPkrbrHTb7tEdxSzLMiA&apiKey=yrjPrIYd0xU9KJpe1xlaR1_K1wFrwc9U-_-99n040JQ",
               url: "https://places.ls.hereapi.com/places/v1/discover/search?in=23.23979546250735%2C77.39215854006432%3Br%3D4717093&addressFilter=stateCode%3DMH&recd=false&size=100&tf=html&X-Mobility-Mode=drive&Accept-Language=en&app_id=hSm0zJfek39BQxeXGRYZ&app_code=46aPkrbrHTb7tEdxSzLMiA",
                 data: { q: request.term },
                 dataType: "json",
                 success: function ($data) {
   ...
   // init_auto_address('INCIDENT_MAP');
    ...
    if(place.address_components){
        ...
        $.each(place.address_components,function(){           
            ...
            if(this.types[0] == 'locality'){
                loc_city = this.long_name;
            }
             if(this.types[0] == 'route'){
                 loc_area = this.long_name;
                $('#add_inc_details_block #area_location').val(this.long_name);
            }
            if(this.types[0] == 'neighborhood'){
                 loc_lmark = this.long_name;
                $('#add_inc_details_block #street_number').val(this.long_name);
            }
     ...
     $('#incident_address_details').val(formatted_address);

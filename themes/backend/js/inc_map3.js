...
function set_inc_add_details(place,map_tagId){
    ...
    //$('#get_ambu_details').attr('href',base_url+'inc/get_inc_ambu?inc_type='+$inc_type+'&lat='+p_lat+'&lng='+p_lng);
        ...
        if(this.types[0] == 'administrative_area_level_1'){
         ...
         if(this.types[0] == 'administrative_area_level_2'){ 
            ...
            $('#add_inc_details #inc_district').val(this.long_name);
             // $dist_dta = "<?php $dt = array('dst_code' => '', 'st_code' => '"+state_code+"', 'auto' => 'inc_auto_addr', 'rel' => 'incient', 'disabled' => ''); echo get_district($dt) ?>";
             //  $('#add_inc_details #incient_dist').html($dist_dta);
...
function update_ambulance_inc_map(){
    ...
    if(jQuery("#DefaultSelectedAmb").length >= 1 ){
        ...
        var amb_id = $('.inc_ambu_list .searched_ambu_item').filter('[data-amb_status="1"]').eq(0).attr('data-amb_id');
       //google.maps.event.trigger($ambMapMarkers[amb_id] , 'click');
        $('.searched_ambu_item#Search_Amb_'+amb_id+' .amb_check_box').click();
    }
...
$(document).ready(function(){
    ...
    $('#container').on('change', '.inc_ambu_list .searched_ambu_item input[type="checkbox"].amb_check_box' , function () {
        ...
        if($distance_value == 'defult distance' || $distance_value == '' ){
            $distance_value = 5;
        }
...
//function get_tahshil_ambulance(){
//    var address =$('#inc_tahshil').parent().find('.mi_autocomplete_input').val();
//     console.log(address);

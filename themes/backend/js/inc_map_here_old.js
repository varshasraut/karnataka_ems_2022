...
//    });
    ...
    var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents($callIncidentMap));
    ...
    $("#inc_map_address").autocomplete({
        select: function( event, ui ) {
         ...
         source: function (request, response) {
             ...
             $.ajax({
                url: "https://autosuggest.search.hereapi.com/v1/autosuggest?at=19.0760,72.8777&apiKey=yrjPrIYd0xU9KJpe1xlaR1_K1wFrwc9U-_-99n040JQ",
                 data: { q: request.term },
                 dataType: "json",
...
//    if(place.latlng){
        var p_lat = place.position.lat;
        var p_lng = place.position.lng;
        ...
        $incMapMarker = new H.map.Marker({lat:p_lat, lng:p_lng});
...
//    
    if($isExtendedMapOpened){
        ...
        $ExtendMapWindow.$incExtMapMarker = new $ExtendMapWindow.H.map.Marker({lat:p_lat, lng:p_lng});
...
function set_inc_ext_pin(){
    ...
    if($ExtendMapWindow.$incExtMapMarkerGroups != null){
        $ExtendMapWindow.$ExtendMap.removeObject($incExtMapMarkerGroups);
        $ExtendMapWindow.$incExtMapMarkerGroups = null;
    }
...
function set_inc_add_details(place,map_tagId){
    ...
    $('#get_ambu_details').attr('href',base_url+'inc/get_inc_ambu?showprocess=yes&inc_type='+$inc_type+'&amb_tp='+$amb_type+'&lat='+p_lat+'&lng='+p_lng);
...
//
//    $('#inc_map_address').val(address);
//     var geocoder = $H_Platform.getGeocodingService(),
//    parameters = {
//      prox: '33.80203,75.197948,250',
...
var $ExtendMap,$ExtendMapUI,$H_Platform_ext;
...
function initExtIncidentMap() {
    ...
    $ExtendMap = new H.Map(
      ...
      var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents($ExtendMap));

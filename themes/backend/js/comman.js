                ...
                $.each(queries, function (c, q) {
                    ...
                    $.each(q, function (c1, q1) {
                        ...
                        if (c1 == 'value') {
                            ...
                            str_value = unescape(q1.toString());
                            
                             input_field_obj.value  = str_value.replace( /(<([^>]+)>)/ig, '');
    ...
    try {
        ...
        for (var it = 0; it < classes.length; it++) {
            ...
            switch (selector[0]) {
                ...
                case "filter_vehical_no":

                


                    if (!/^[A-Z]{2}[ -][0-9]{1,2}[ -][A-Z]{1,2}[ -][0-9]{4}$/.test(element_val)) {
                ...
                case "filter_password":
                    ...
                    if (regex.exec(element_val) == null) {
                      $.is_error = 1;
                    }
                ...
                case "filter_float": //if(!/[0-9]/.test(element_val)){ $.is_error = 1;}
                    ...
                    case "filter_integer": //if(!/[0-9]/.test(element_val)){ $.is_error = 1;}
...
function show_popup(htmltext, ppwidth, ppheight, ismessage) {
    ...
    if ($('#container').find('#mi_global_popup').length <= 0) {
        ...
        $('#popupconitainer').prepend('<div id="mi_global_popup"   data-popupwidth="' + ppwidth + '" data-popupheight="' + ppheight + '">Loading...</div>');
    ...
    if ($('#container').find('#mipopup').length <= 0) {
        ...
        $('#container').prepend("<a id='mipopup' href='#mi_global_popup'></a>");
    ...
    if (ismessage) {
        ...
        htmltext = htmltext + '<div align="center"  style="clear:both; width:100%"><br/><input name="btnclosebox" value="Ok" class="btnclosemipopup"  type="button"></div>';
        ...
        if (SUCCESS_NOTIFATION_FLAG == 0) {
            ...
            if (res.message != "" && res.showprocess != "no") {
                ...
                setTimeout(function () {
                    ...
                    show_popup(res.message, 400, 250, true);
...
//					  alert(res.popup);
        ...
        if (res.position != "") {
            ...
            $.each(res.position, function (resposition, respositionhtml) {
                //console.log(request_setting.output_position);
    ...
    $('#container').on('click', '.form-xhttp-request', function () {
        ...
        if ($(this).closest("form")) {
            ...
            if (errorcnt == 0) {
                ...
                $(".save_btn_wrapper .accept_btn").prop("disabled", true);
...
//                });
    ...
    $('#container').on('focus', '.mi_timecalender', function () {
        ...
        $(this).datetimepicker({
         ...
         $(this).datepicker( "fast" );

        //return false;

    });

    $('#container').on('focus', '.mi_timecalender_sch', function () {
    ...
    $('#container').on('focus', '.mi_timecalender_cur', function () {
        var dateToday = new Date();
        $(this).datepicker( "destroy" );
         event.preventDefault();
        $(this).datetimepicker({
    ...
    $('#container').on('focus', '.mi_timecalenderoff', function () {
        $(this).datepicker( "destroy" );
         event.preventDefault();
        $(this).datetimepicker({
            changeMonth: true,
  ...
  $('#container').on('focus', '.onroad_datetime', function () {
    var $break_datetime = $("#offroad_datetime").val();
    var $fmdate = new Date($break_datetime);

// alert($fmdate);
    $(".onroad_datetime" ).datepicker( "destroy" );
    $('.onroad_datetime').last().datepicker('refresh');
    $('.onroad_datetime').datetimepicker({
        dateFormat: "yy-mm-dd",
        timeFormat: "HH:mm:ss",
        minDate: $fmdate,
    ...
    $('#container').on('focus', '.mi_timecalenderon', function () {
    ...
    $('#container').on('focus', '.mi_next_timecalender', function () {
    ...
    $('#container').on('focus', '.mi_cur_timecalender', function () {
        $(this).datepicker( "destroy" );
         event.preventDefault();
        $(this).datetimepicker({
            changeMonth: true,
    ...
    $('#container').on('focus', '.mi_cur_date', function () {
        $(this).datepicker( "destroy" );
         event.preventDefault();
        $(this).datepicker({
            changeMonth: true,
    ...
    $('#container').on('focus', '.mi_cur_year_date', function () {
        $(this).datepicker( "destroy" );
         event.preventDefault();
        $(this).datepicker({
            changeMonth: true,
    ...
    $('#container').on('click', '.onpage_popup', function () {
        ...
        if ($('#container').find('#popup_div').length <= 0) {
            ...
            $('#popupconitainer').append('<div id="popup_div">Loading...</div>');
    ...
    $('#container').on('mousedown', '.minimize_scroll_menu , ul.incoming_calls li', function () {
	    ...
	    $('#container').on('mousedown', '.minimize_scroll_menu_pda', function () {

        $('#pda_incoming_call_nav').toggleClass('isOut');

        var isOut = $('#pda_incoming_call_nav').hasClass('isOut');

        $('#pda_incoming_call_nav').animate({
            right: isOut ? '-123' : '0'

        }, 300);

        $('#pda_incoming_call_nav .minimize_scroll_menu_pda .head').toggleClass('head_revers');
...
$(window).load(function () {
    ...
    incoming_call_timer = setInterval(change_incoming_call, 10000);

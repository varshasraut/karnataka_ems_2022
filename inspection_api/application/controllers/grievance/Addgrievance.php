...
class Addgrievance extends REST_Controller {
    ...
    public function index_post(){
        ...
        if($auth == 1){
            ...
            $gri['status'] = '0';
            $insp['gri_status'] = '1';
            $this->Grievance_model->closedGriv($insp,$insp_id);
    ...
    public function CompletedGrievanceList_post(){
        $data['login_secret_key'] =  $this->post('loginSecretKey');
        $data['device_id'] =  $this->post('uniqueId');
        $data['username'] =  $this->post('username');
        ...
        $auth = $this->Login_model->checkLoginUserforAuth($data);
        if($auth == 1){
            $griList = $this->Grievance_model->getGrievanceCompletedList($username);
            $this->response([
                'data' => $griList,
                'error' => null
            ],REST_Controller::HTTP_OK);
    ...
    public function grievanceListInPro_post(){
        $data['login_secret_key'] =  $this->post('loginSecretKey');
        $data['device_id'] =  $this->post('uniqueId');
        $data['username'] =  $this->post('username');
        $username =  $this->post('username');
        $auth = $this->Login_model->checkLoginUserforAuth($data);
        if($auth == 1){
            $griList = $this->Grievance_model->getgrievanceListInPro($username);
            $this->response([
                'data' => $griList,
                'error' => null
            ],REST_Controller::HTTP_OK);
    ...
    public function grievanceDetails_post(){
        $data['login_secret_key'] =  $this->post('loginSecretKey');
        $data['device_id'] =  $this->post('uniqueId');
        $data['username'] =  $this->post('username');
        $griUniqeId =  $this->post('griUniqeId');
        $auth = $this->Login_model->checkLoginUserforAuth($data);
        if($auth == 1){
            $griList = $this->Grievance_model->getGrievanceDetails($griUniqeId);
            $this->response([
                'data' => $griList[0],
                'error' => null
            ],REST_Controller::HTTP_OK);

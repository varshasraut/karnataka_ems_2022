...
class Erc_reports extends EMS_Controller {
    ...
    function load_erc_report_form() {
        ...
        if ($report_type == 'Incident_Reports') {
            ...
            $data['zones'] = $this->common_model->get_divisions_data();
        ...
        if ($post_reports['reports'] == 'view') {
           ...
           // var_dump($report_data);die();
            ...
            // echo $time3;die; 
                    ...
                    if($row['scene_odometer']=='' || $row['scene_odometer']==null){ $scene_odo = 0;}
                    else{ $scene_odo = (int)$row['scene_odometer'];}
    
                    if($row['start_odo']=='' || $row['start_odo']==null){ $start_odo = 0;}
                    else{ $start_odo = (int)$row['start_odo'];}
    
                    
                    $B2Skm = $scene_odo - $start_odo;
    
                    if($row['hospital_odometer']=='' || $row['hospital_odometer']==null){ $hos_odo = 0;}
                    else{ $hos_odo = (int)$row['hospital_odometer'];}
            ...
            $B2BTime =  date('H:i:s', strtotime($B2BTime->format("%h:%i:%s")));
                ...
                $inc_data = array(
                    ...
                    'provider_impressions' => $provider_impressions,
                    'ayushman_id'=> $row['ayushman_id'],
                    'drop_district1'=> $row['drop_district'],

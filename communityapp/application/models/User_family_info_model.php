...
class User_family_info_model extends CI_Model {
    ...
    public function get_user_all_fam_info($usrMobNo){
        $this->db->select('usr.mobile,usr.usr_id,fam.fam_id,fam.fam_f_name,fam.fam_l_name,fam.fam_age,fam.fam_age_grp,fam.fam_blood_grp,fam.fam_email,fam.fam_mobile,fam.fam_gender,fam.fam_ayushman_id,fam.fam_address,blood.bldgrp_name');
        $this->db->from('ems_comu_app_user_reg usr');
        $this->db->join("ems_comu_app_family_info fam","usr.usr_id = fam.usr_id","left");
        $this->db->join('ems_mas_bloodgrp_type blood',"fam.fam_blood_grp = blood.bldgrp_id","left");
    ...
    public function get_user_fam_info_as_id($usrFamId){
        $this->db->select('fam.fam_f_name,fam.fam_l_name,fam.fam_age,fam.fam_age_grp,blood.bldgrp_name,fam.fam_email,fam.fam_mobile,fam.fam_gender,fam.fam_ayushman_id,fam.fam_address');
        $this->db->from('ems_comu_app_family_info fam');
        $this->db->join('ems_mas_bloodgrp_type blood',"fam.fam_blood_grp = blood.bldgrp_id","left");
        $this->db->where('fam.fam_is_deleted','0');
        $this->db->where('fam.fam_id',$usrFamId);
        $query = $this->db->get();
        if ($query->num_rows() > 0 )
        {
            return $query->result_array();

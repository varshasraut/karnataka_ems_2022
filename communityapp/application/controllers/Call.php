...
class Call extends REST_Controller {
    ...
    public function maplink_post() {
        ...
        if($auth == 1){
            ...
            if(empty($linkdetails)){
                ...
                $data1 = array();
                //$linkdetails1['track_status'] : 0 is enable and 1 is disable
                foreach($linkdetails as $linkdetails1){
                    $data = array(
                        "link" => $linkdetails1['track_link'],
                        "status" => $linkdetails1['track_status']
    ...
    public function ambulancerequest_post(){
        $chkLoging['login_secret_key'] =  $this->post('loginSecretKey');
        $chkLoging['uniqueId'] =  $this->post('uniqueId');
        $chkLoging['mobile'] =  $this->post('mobile');
        ...
        if($patient == ""){
            $userMobNo =  $this->post('mobile');
            $userId = $this->Call_model->getUserMobId($userMobNo);
            // print_r($userId);
            $patientId = $userId[0]['usr_id'];
        }else{
            $patientId = $patient;
        }
        if($auth == 1){
            $ramNo = rand(1000, 9999);
            $date = date('Ymds');
            $req['user_req_unique_id'] = $date.$ramNo;
            $req['user_req_calltype'] = $this->post('callType');
            $req['user_req_chiefcomplaint'] = $this->post('chiefComplaint');
            $req['user_req_hosptype'] = $this->post('hosptype');
            $req['user_req_hospid'] = $this->post('hospId');
            $req['user_req_usertype'] = $this->post('userType'); //1 : itself and 2 : family member
            $req['user_req_patient'] = $patientId;
            $req['user_req_mobile'] = $this->post('mobile');
            $req['user_req_userlat'] = $this->post('userlat');
            $req['user_req_userlng'] = $this->post('userlng');
            $req['user_req_added_date'] = date('Y-m-d');
            $req['user_req_dispatched '] = 'Request';
            // print_r($req);die;
            $reqRec = $this->Call_model->addAmbRequest($req);
            if($reqRec == 1 ){
                $this->response([
                    'data' => ([
                        'code' => 1,
    ...
    public function ambulancerequestlist_post(){
        $chkLoging['login_secret_key'] =  $this->post('loginSecretKey');
        $chkLoging['uniqueId'] =  $this->post('uniqueId');
        $chkLoging['mobile'] =  $this->post('mobile');
        $userMobNo = $this->post('mobile');
        $auth = $this->Login_model->checkLoginUserforAuth($chkLoging);
        $rec = $this->Call_model->getAmbReqData($userMobNo);
        if(empty($rec)){
            $rec1 = [];
        }else{
            $rec1 = $rec;
    ...
    public function ambulancerequestdispatch_post(){
        $chkLoging['login_secret_key'] =  $this->post('loginSecretKey');
        $chkLoging['uniqueId'] =  $this->post('uniqueId');
        $chkLoging['mobile'] =  $this->post('mobile');
        $userMobNo = $this->post('mobile');
        $auth = $this->Login_model->checkLoginUserforAuth($chkLoging);
        $rec = $this->Call_model->getAmbReqDisData($userMobNo);
        // print_r($rec);
        if(empty($rec)){
            $rec1 = [];
        }else{

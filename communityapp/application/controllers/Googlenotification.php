...
class Googlenotification extends REST_Controller {
    ...
    public function index_post(){
        ...
        file_put_contents('../logs/comunotification.log', $php_input);
        
        define('API_ACCESS_KEY','AAAAZdgArPU:APA91bGqF-aiN3oQO0e5EAW3LCyT8GoHral2gkVeervdZ-jkw_a7doqq3RM9aeSyHvhgILh4I0hu_bdrRkZRAeCj9UgQrvRSMPKzgeVR96emEJB1UbL-TtmEvVRt1BW9HCC456tLesCt');
        $fcmUrl = 'https://fcm.googleapis.com/fcm/send';
        // $data = $this->user->getLoginAmb();
        $user_mob_no = $post->userMobileNo;
        $amb_no = $post->ambulanceNo;
        // $token = $this->post('token');
        $incidentId = $post->incidentId;
        $cheifComplaint = $post->cheifComplaint;
        $data = $this->Login_model->chkUserLogin($user_mob_no);
        // print_r($data);die;
        for($i=0;$i<count($data);$i++){
            ...
            // print_r($token);
            $notification = [
                'title' =>'Assinged Ambulance For Your Request',
                'incidentId' => $incidentId,
                'discription' => 'Incident Id : '.$incidentId,
                'message' => 'New Ambulance Assigned',
                'ambulanceNo' => $amb_no,
            ...
            $notification = [
                'title' =>'Assinged Ambulance For Your Request',
                'incidentId' => $incidentId,
                "android_channel_id" => "mp_dial_108_community_app",
                'discription' => 'Incident Id : '.$incidentId,
                'body' => 'This is ambulance number ' .$amb_no,
                'ambulanceNo' => $amb_no,
                'cheifComplaint' => $cheifComplaint
            ];
            $fcmNotification = [
            ...
            // print_r(json_encode($fcmNotification));die;
            $headers = [

...
class Login extends REST_Controller {
    ...
    public function usrregister_post(){
        ...
        $data['age_grp'] = $this->post('ageGroup');
        $data['blood_grp'] = $this->post('bloodGrp');
        $data['uer_passward'] = MD5($this->post('password'));
        $data['user_ayushman_id'] = $this->post('ayushmanId');
        ...
        if($usrReg == 1){
            ...
            $sms_autofill = $this->post('smsAutofill');
            $otp = rand(1000, 9999);
            $mobile = $this->post('mobile');
            $txtMsg='';
            $txtMsg .=  "".$otp." otp as your verification code on Sanjeevani108 Application."." \n";
            $txtMsg .=  "The OTP expires within 10 mins. ";
            $txtMsg .= "".$sms_autofill." JAES"." \n";
            $text_msg = urlencode($txtMsg);
            $form_url = "http://hindit.biz/api/pushsms?user=jaes&authkey=92zRsmUQr4uUs&sender=JAESPL&mobile=$mobile&text=$text_msg&entityid=1701164198802150041&templateid=1707165797175799982&rpt=1&summary=1&output=json";
            $data_to_post = array();
            $curl = curl_init();
            ...
            $this->Login_model->insertUsrReg($data);
            $this->response([
                'data' => ([
                    'code' => 2,
    ...
    public function chkotp_post(){
        ...
        $current_time = date('Y-m-d H:i:s');
        $usrData = $this->Common_model->getAllUsrData($data);
        ...
        $mobile = $this->post('mobile');
        // print_r($usrLoginData);
        if(is_array($usrLoginData) ){
            if($usrLoginData[0]['unique_id'] == $uniqueId && $data['mobile'] == $usrLoginData[0]['mobile']){
                $this->response([
                    'data' => null,
                    'error' => ([
                        'code' => 4,
            ...
            }else if($usrLoginData[0]['unique_id'] != $uniqueId && $data['mobile'] == $usrLoginData[0]['mobile']){
                $logoutOtherDeviceUser = $this->Login_model->logoutOtherDeviceUser($mobile);
                if($logoutOtherDeviceUser == 1){
            ...
            if($otpPass == 1){
                if(($usrData[0]['otp'] == $otp)){
                    if(($usrData[0]['otp_expire_time'] >= $current_time)){
                        if(is_array($usrLoginData) ){
                            if($usrLoginData[0]['unique_id'] != $uniqueId && $data['mobile'] == $usrLoginData[0]['mobile']){
                                $logoutOtherDeviceUser = $this->Login_model->logoutOtherDeviceUser($data['mobile']);
                                if($logoutOtherDeviceUser == 1){
                                    $this->Login_model->usrLogin($data1);
                                    $this->response([
                                        'data' => ([
    ...
    public function signin_post(){
        ...
        $otpPass = $this->post('otpPass'); //otp :1 and pass: 2
        $sms_autofill = $this->post('smsAutofill');
        $usrReg = $this->Common_model->getAllUsrData($data);
        if($otpPass == 1){
            if(!empty($usrReg)){
                $otp = rand(1000, 9999);
		        //$otp = '1994';
                ...
                $txtMsg .= "".$sms_autofill." JAES"." \n";
                $text_msg = urlencode($txtMsg);
                $form_url = "http://hindit.biz/api/pushsms?user=jaes&authkey=92zRsmUQr4uUs&sender=JAESPL&mobile=$mobileNo&text=$text_msg&entityid=1701164198802150041&templateid=1707165797175799982&rpt=1&summary=1&output=json";
                $data_to_post = array();
                $curl = curl_init();
    ...
    public function logout_post(){
        ...
        $data['mobile'] =  $this->post('mobile');
        $activate = $this->post('activate');
        $userMob = $this->post('mobile');
        $auth = $this->Login_model->checkLoginUserforAuth($data);
        if($auth == 1){
            $log = $this->Login_model->logout($data,$activate);
            if($activate == 1){
                $this->Login_model->userAccDeactivation($activate,$userMob);
            }
            if($log == 1){
                $this->response([
                    'data' => ([

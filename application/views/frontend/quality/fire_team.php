            ...
            <form enctype="multipart/form-data" action="#" method="post" id="add_inc_details">
                ...
                <div class="width100 float_left">
                    <div class="width100 form_field outer_smry">
                        ...
                        <div class="width100 float_left">
                            <div class="label blue float_left width30">FDA Supervisor</div>

                            <div class="width2 float_left width70" id="ero_supervisor">
                                 <input name="team[ero_supervisor]" class="mi_autocomplete" data-href="<?php echo base_url();?>auto/get_auto_clg?clg_group=UG-FDASupervisor"  data-value="<?php echo $stud_sickroom[0]->ero_supervisor ;?>" value="<?php echo $stud_sickroom[0]->ero_supervisor ;?>" type="text" tabindex="2" placeholder="FDA Supervisor" data-callback-funct="load_fire_by_supervisor">
                            </div>
                        </div>
                    </div>
                    <div id="ERO_list">
...
<!--                <input type="button" name="submit" value="<?php if ($update) { ?>Update<?php } else { ?>Submit<?php } ?>" class="btn submit_btnt form-xhttp-request" data-href='<?php echo base_url();?>quality/<?php if ($update) { ?>update_fire_team<?php } else { ?>save_fire_team<?php } ?>' data-qr='output_position=content&amp;prof=<?php echo @$prof; ?>&amp;module_name=quality' TABINDEX="23" id="<?php echo @$current_data[0]->clg_ref_id; ?>">-->
                   <input type="button" name="submit" value="<?php if ($update) { ?>Update<?php } else { ?>Submit<?php } ?>" class="btn submit_btnt form-xhttp-request" data-href='<?php echo base_url();?>quality/<?php if ($update) { ?>update_fire_team<?php } else { ?>save_fire_team<?php } ?>' data-qr='output_position=content&amp;prof=<?php echo @$prof; ?>&amp;module_name=quality' TABINDEX="23" id="<?php echo @$current_data[0]->clg_ref_id; ?>">

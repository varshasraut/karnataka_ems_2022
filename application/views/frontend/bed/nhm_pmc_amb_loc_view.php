...
var markerins_pmc = new Array();
var amblatlong_pmc = new Array();
<?php

if (is_array($pmc_amb_data)) {
	$ambids_pmc =  array();

    foreach ($pmc_amb_data as $amb) {
       ...
       amblatlong_pmc['<?php echo $ambid?>'] = {title:"<?php echo trim($amb->amb_rto_register_no); ?>", lat:parseFloat(<?php echo $amb->amb_lat; ?>), lng:parseFloat(<?php echo $amb->amb_log; ?>),info:'<?php echo base64_encode($amb_info);?>',amb_status:"<?php echo $amb->amb_status;?>",amb_type:"<?php echo ($amb->amb_type);?>",thirdparty:"<?php echo ($amb->thirdparty);?>"};
        
    <?php
        
    }
	$ambids_pmc = join(",",$ambids_pmc);
}
?>
  </script>
    ...
    <div class="row">
        <div class="col-md-12"> 
         <div class="ems_amb_map" style="width: 100%; height: 100%; position:absolute;left: 0%; "  data-inc_lat="<?php echo $inc_data[0]->inc_lat;?>" data-inc_lng="<?php echo $inc_data[0]->inc_long;?>"> 
            ...
            <div id="PMC_INC_MAP" class="" style="width: 100%; height: 400px; background: #5A8E4A; position:absolute; left: 0%; " data-ambids-pmc="<?php echo $ambids_pmc?>">
...
<script>
    ...
    var map_obj_pmc,$google_map_obj_pmc;
	var post_data_url = { url: base_url +'amb/gpsapi',type: "POST",  cache: false};
    
    var $directionsAmbPMC;
    var $directionsDisplayAmbPMC;
    var $AllMapUIPMC, $AllMapPMC;
    var $AllgroupPMC =null;

   function initmarker2(){
	   
	       
                ambids = $("#PMC_INC_MAP").attr('data-ambids-pmc');
...
//                $destination = inc_lat+','+inc_lng;
			   ...
			   for(ambin=0; ambin < ambids.length ; ambin++ ){
                ...
                if(parseFloat(amblatlong_pmc[ambids[ambin]].thirdparty) == '2' || parseFloat(amblatlong_pmc[ambids[ambin]].thirdparty) == 2){
                    if(parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == '2' || parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == 2){

                    map_mark_pin = red_102_mark_pin;
                    }else if(parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == '6' || parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == 6){

                    map_mark_pin = pink_mark_pin;
                    }else if(parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == '1' || parseFloat(amblatlong_pmc[ambids[ambin]].amb_status) == 1){
...
console.log("2");
                ...
                var markerLatLng = {lat: parseFloat(amblatlong_pmc[ambids[ambin]].lat), lng: parseFloat(amblatlong_pmc[ambids[ambin]].lng)};
               //console.log(markerLatLng);
              //  console.log({lat: parseFloat(amblatlong[ambids[ambin]].lat), lng: parseFloat(amblatlong[ambids[ambin]].lng)});
              //addMarkersToMap(markerLatLng, map_mark_pin, $AllMapPMC, atob(amblatlong[ambids[ambin]].info));
              addInfoBubble2(markerLatLng, map_mark_pin, atob(amblatlong_pmc[ambids[ambin]].info));
 ...
 function set_marker_window2($maker,$msg){
 ...
 function resetmarker2($google_map_obj_pmc,ambregno,lat,long){
	  ...
	  if(ambregno!=""){
	    ...
	    if (markerins_pmc[ambregno] != null) {
		 ...
		 markerins_pmc[ambregno].setPosition(markerLatLng);
         
          //set_amb_direction($google_map_obj,markerLatLng,$destination);
	  }
	 }
function addInfoBubble2(markerLatLng, map_mark_pin, html) {
    //console.log(markerLatLng);
    if(markerLatLng == '' || markerLatLng == null){
        return false;
    }
    
    $AllgroupPMC = new H.map.Group();
    $AllMapPMC.addObject($AllgroupPMC); 
    ...
    parisMarker.addEventListener('pointerenter', function (evt) {
      ...
      $AllMapUIPMC.getBubbles().forEach(bub => $AllMapUIPMC.removeBubble(bub));
      $AllMapUIPMC.addBubble(bubble);
    }, false);
    $AllgroupPMC.addObject(parisMarker);
    $AllMapPMC.getViewModel().setLookAtData({
    ...
    function addMarkersToMap2($AllgroupPMC, markerLatLng, map_mark_pin, $AllMapPMC, html) {
      // console.log("markerLatLng" + markerLatLng);
        ...
        $AllgroupPMC.addObject(parisMarker);
        $AllMapPMC.getViewModel().setLookAtData({
            bounds: $AllgroupPMC.getBoundingBox()
        });
 ...
 function loadmap2() {
    ...
    $AllMapPMC = new H.Map( document.getElementById('PMC_INC_MAP'),
    ...
    var behavior = new H.mapevents.Behavior(new H.mapevents.MapEvents($AllMapPMC));

    $AllMapPMC.tagId = 'ALL_MAP';
    
    window.addEventListener('resize', () => $AllMapPMC.getViewPort().resize());
    $AllMapUIPMC = H.ui.UI.createDefault($AllMapPMC, defaultLayers);
    ...
    addInfoBubble2(markerLatLng="", map_mark_pin="", html="");       
					   ...
					   var gpsdata = JSON.parse(result);
			             $.each(gpsdata, function (index, ambdata) {
												   try{
														...
														resetmarker2($google_map_obj_pmc,ambdata.amb_rto_register_no,ambdata.amb_lat,ambdata.amb_log); 
...
function set_amb_direction2($google_map_obj_pmc,$origin,$destination) {
  
  
    var $directionsAmbPMC = new google.maps.DirectionsService();
    var $directionsDisplayAmbPMC = new google.maps.DirectionsRenderer({suppressMarkers: true});
   
   $directionsDisplayAmbPMC.setMap($google_map_obj_pmc);
     //console.log($origin);
    ...
    $directionsAmbPMC.route(request, function(result, status) {
        
        if (status == 'OK') {
            
            $directionsDisplayAmbPMC.setDirections(result);
        }
    });

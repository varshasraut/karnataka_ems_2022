...
if (is_array($amb_data)) {
    ...
    foreach ($amb_data as $amb) {
        ...
        if($amb->amb_status == 2 ){
           /* if($amb->parameter_count == '1'){
                $time = $amb->start_from_base_loc;
                $satus = 'Start From Base';
            }elseif($amb->parameter_count == '2'){
                $time = $amb->at_scene;
                $satus = 'At Scene';
            }elseif($amb->parameter_count == '3'){
                $time = $amb->from_scene;
           ...
           echo '<strong>Pilot Mobile No: </strong><a class="click-xhttp-request" style="color:#000;" data-href="{base_url}avaya_api/soft_dial" data-qr="output_position=content&mobile_no=0'.$amb->amb_pilot_mobile.'">'.$amb->amb_pilot_mobile.'</a>';
            ...
            echo '<br>';
            echo  '<strong>Current Status: </strong>'. $satus; 
            echo '<br>';
            echo  '<strong>Current Status Time: </strong>'. $amb->time; 
            echo '</div>';
          }else{ 
    ...
    <div class="row">
        <div class="col-md-12"> 
         <div class="ems_amb_map" style="width: 100%; height: 100%; position:absolute;left: 0%; "  data-inc_lat="<?php echo $inc_data[0]->inc_lat;?>" data-inc_lng="<?php echo $inc_data[0]->inc_long;?>"> 
            ...
            <div id="INC_MAP" class="" style="width: 100%; height: 500px; background: #5A8E4A; position:absolute; left: 0%; " data-ambids="<?php echo $ambids?>">

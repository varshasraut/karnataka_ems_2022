...
<?php if($amb_id){ 
    if($amb_id=='all'){ 
        ...
        $heading = 'MPEMS 102 Ambulances';
    }else if($amb_id='Single_amb'){
        $heading = 'MPEMS Ambulance Search :';
        $heading = $heading. ' '.$amb_rto_register;
   }

} else { $heading = 'MPEMS Ambulances'; }?>
<div>
...
<select id="amb_id"   name="amb_reg_id"  onchange="amb_typewise_load()">
    ...
    <option value="102amb">102 Ambulance</option>
    ...
    foreach ($amb_data as $amb) {
        ...
        if($amb->amb_status == 2 ){
            ...
            if($amb->inc_complaint != '' && $amb->inc_complaint != 0){
                echo  '<strong>Cheif Complaint: </strong>'. get_cheif_complaint($amb->inc_complaint); 
                echo '<br>';
            }
            if($amb->inc_complaint_other != ''){
                echo  '<strong>Cheif Complaint Other: </strong>'. $amb->inc_complaint_other; 
                echo '<br>';
            }
       ...
       amblatlong['<?php echo $ambid?>'] = {title:"<?php echo trim($amb->amb_rto_register_no); ?>", lat:parseFloat(<?php echo $amb->amb_lat; ?>), lng:parseFloat(<?php echo $amb->amb_log; ?>),info:'<?php echo base64_encode($amb_info);?>',amb_status:"<?php echo $amb->amb_status;?>",amb_type:"<?php echo ($amb->amb_type);?>"};
    ...
    <div class="row">
        <div class="col-md-12"> 
         <div class="ems_amb_map" style="width: 100%; height: 100%; position:absolute;left: 0%; "  data-inc_lat="<?php echo $inc_data[0]->inc_lat;?>" data-inc_lng="<?php echo $inc_data[0]->inc_long;?>"> 
            ...
            <div id="INC_MAP" class="" style="width: 100%; height: 400px; background: #5A8E4A;position:absolute;left: 0%; " data-ambids="<?php echo $ambids?>">

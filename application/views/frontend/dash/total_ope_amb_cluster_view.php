                ...
                <table class="table report_table">
                    ...
                    if($atc_screening){
                        ...
                        foreach($atc_screening as $key=>$cluster){ 
                            $chart_value[] = $cluster['total_onroad_amb'];
...
<!--                            <td><a class="click-xhttp-request" data-href="{base_url}dash/<?php echo $school_link;?>" data-qr="output_position=content&atc=<?php echo $atc;?>&po=<?php echo $po;?>&cluster_id=<?php //echo $cluster['cluster_id'];?>"><?php //echo $cluster['cluster_name'];?></a></td>  -->
                        ...
                        <td><?php echo $cluster['total_amb']?$cluster['total_amb']:0;?></td>  
                        <td><?php echo $cluster['total_onroad_amb']?$cluster['total_onroad_amb']:0;?></td>   
                        <td><?php if($cluster['total_amb'] >0 ) { echo ($cluster['total_onroad_amb']/$cluster['total_amb'])*100; } else { echo '0';}?>%</td>    
                        </tr>
                      <?php 
                      
                        $total = $total+$cluster['total_amb'];
                        $total_onroad_amb= $total_onroad_amb+$cluster['total_onroad_amb'];
                        }

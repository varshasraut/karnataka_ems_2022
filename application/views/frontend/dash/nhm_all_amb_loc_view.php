...
<?php if ($amb_id) {
    if ($amb_id == 'all') {
        $heading = 'All Ambulance';
    } else if ($amb_id == '108amb') {
        $heading = '108 Ambulances';
    } else if ($amb_id == '102amb') {
        $heading = 'JE Ambulances';
    }  else if ($amb_id = 'Single_amb') {
        $heading = 'MPEMS Ambulance :';
        $heading = $heading . ' ' . $amb_rto_register;
    }
} else {
    $heading = 'MPEMS Ambulance';
}
...
<div class="row">
    <div class="col-md-3">
        <label class="headerlbl"><?php echo $heading; ?></lable>
    </div>
    <div class="col-md-3">
        <?php $available = "347"; ?>
        <label class="headerlbl"><img src="<?php echo base_url(); ?>themes/backend/images/select_icon_pin.png" alt="Available" width="30" height="30">&nbsp;&nbsp;<?php echo 'Available'; ?> <?php echo $amb_available; //echo $total_amb_data[0]->off_road_doctor;
                                                                                                                                                                                                ?></label>
    </div>
    <div class="col-md-3">
        <label class="headerlbl"><img src="<?php echo base_url(); ?>themes/backend/images/red_map_pin_small.png" alt="On Call" width="30" height="30">&nbsp;&nbsp;<?php echo 'On Call'; ?> <?php echo $amb_busy; ?> </label>
    </div>
    <div class="col-md-3">
        <label class="headerlbl"><img src="<?php echo base_url(); ?>themes/backend/images/blue_select_icon_pin.png" alt="Maintenance" width="30" height="30">&nbsp;&nbsp;<?php echo 'Maintenance'; ?> <?php echo $amb_maintainance; ?></label>
    </div>
</div>
<div class="row">
    <div class="col-md-3"><label for=""> Select Ambulance :</label></div>
    <div class="col-md-3">
        <!--<input name="amb_reg_id"  id="amb_id" class="mi_autocomplete dropdown_per_page width97" data-href="{base_url}auto/get_ambulance_dash" placeholder="Select Ambulance" data-errors="{filter_required:'Please select state from dropdown list'}" tabindex="2" autocomplete="off" value="<?php echo $amb_rto_register; ?>" data-qr="" data-value="<?php echo $amb_rto_register; ?>" data-callback-funct="amb_typewise_load">-->
        <select id="amb_id" name="amb_reg_id" onchange="amb_typewise_load()">
            <!--<option value="">Select Type</option>-->
            <option value="all" <?php if ($amb_rto_register ==  'all') {
                                    echo "selected";
                                } ?>>All Ambulance</option>
            <!--<option value="104amb" <?php if ($amb_rto_register ==  '104amb') {
                                    echo "selected";
                                } ?>>104 Ambulance</option>-->
            <option value="102amb" <?php if ($amb_rto_register ==  '102amb') {
                                    echo "selected";
                                } ?>>JE Ambulances</option>
            <option value="108amb" <?php if ($amb_rto_register ==  '108amb') {
                                        echo "selected";
                                    } ?>>108 Ambulances</option>
            <!--<option value="bikeamb">Bike Ambulance</option>-->
            <option value="single_amb" <?php if ($amb_rto_register ==  'single_amb') {
                                            echo "selected";
                                        } ?>>Ambulance select</option>

        </select>

    </div>

    <div class="col-md-2" id="single_amb" <?php if ($amb_id_red == '') {
                                                echo 'style="display:none"';
                                            } ?>>

        <input name="amb_reg_id" id="ambulance_id" class="mi_autocomplete dropdown_per_page width97" data-href="{base_url}auto/get_ambulance" placeholder="Select Ambulance" data-errors="{filter_required:'Please select state from dropdown list'}" tabindex="2" autocomplete="off" value="<?php echo $amb_id_red; ?>" data-value="<?php echo $amb_id_red; ?>" data-callback-funct="single_ambulance_load_dash">
    ...
    <div class="col-md-2">
        <select name="amb_district" id="amb_district" style="" data-errors="{filter_required:'dropdown should not blank'}" TABINDEX="7" onchange="amb_typewise_load()">
            <option value="">All</option>
            <!-- <option value="">All</option> -->
            <?php foreach ($dist as $dst) { ?>
                <option value="<?php echo $dst->dst_code; ?>" <?php if ($district_id ==  $dst->dst_code) {
                                                                    echo "selected";
...
<script type="text/javascript">
    ...
    if (is_array($amb_data)) {
        ...
        foreach ($amb_data as $amb) {
            ...
            $ambids[] = $ambid;
            $emso_id = get_amb_login_status($amb->amb_id);
            $pilot_no = '';
            $emso_no = '';
            if ($emso_id) {
            ...
            if ($amb->amb_status == 2) {
                ...
                echo '<div style="width:250px;background-color: white"><strong>' . $amb->hp_name . '</strong>';
                echo  '<br>';
            ...
            echo '<br>';
                ...
                echo '<br>';
                echo  '<strong>Patient Name: </strong>' . $amb->ptn_fname . ' ' . $amb->ptn_lname;
                echo '<br>';
                echo  '<strong>Gender: </strong>' . get_gen($amb->ptn_gender);
                echo '<br>';
                echo  '<strong>Age: </strong>' . $amb->ptn_age . ' ' . $amb->ptn_age_type;
                echo '<br>';
                echo '<strong>Pilot Mobile No(O): </strong><a class="click-xhttp-request" style="color:#000;" data-href="{base_url}avaya_api/soft_dial" data-qr="output_position=content&mobile_no=0' . $amb->amb_pilot_mobile . '">' . $amb->amb_pilot_mobile . '</a>';
                echo '<br>';
                echo '<strong>Pilot Mobile No(P): </strong>' . $pilot_no;
                echo '<br>';
                echo '<strong>EMT Mobile No(O): </strong><a class="click-xhttp-request" style="color:#000;" data-href="{base_url}avaya_api/soft_dial" data-qr="output_position=content&mobile_no=0' . $amb->amb_default_mobile . '">' . $amb->amb_default_mobile . '</a>';
                echo '<br>';
                echo '<strong>EMT Mobile No(P): </strong>' . $emso_no;

                echo '</div>';
            } else {

                echo  '<div style="width:300px;background-color: white"><strong>' . $amb->hp_name . '</strong>';
                echo '<br>';
                echo '<strong>RTO No: </strong>' . $amb->amb_rto_register_no;
                echo '<br>';
                // echo '<strong>Ambulance Status: </strong>'.show_amb_status_name($amb->amb_status); 
                echo '<strong>Ambulance Name: </strong>'.$amb->ambt_name; 
    ...
    ?>
            ...
            amblatlong['<?php echo $ambid ?>'] = {
                title: "<?php echo trim($amb->amb_rto_register_no); ?>",
                lat: parseFloat(<?php echo $amb->amb_lat; ?>),
                lng: parseFloat(<?php echo $amb->amb_log; ?>),
                info: '<?php echo base64_encode($amb_info); ?>',
                amb_status: "<?php echo $amb->amb_status; ?>",
                amb_type: "<?php echo ($amb->amb_type); ?>",
                amb_user: "<?php echo ($amb->amb_user); ?>"
            };
...
<div class="row">
    <div class="col-md-12">
        <div class="ems_amb_map" style="width: 100%; height: 100%; position:absolute;left: 0%; " data-inc_lat="<?php echo $inc_data[0]->inc_lat; ?>" data-inc_lng="<?php echo $inc_data[0]->inc_long; ?>">
            ...
            <div id="INC_MAP" class="" style="width: 100%; height: 80vh; background: #5A8E4A;position:absolute;left: 0%; " data-ambids="<?php echo $ambids ?>">
...
<script>
    ...
    var pink_mark_pin = new H.map.Icon(base_url + "themes/backend/images/blue_select_icon_pin.png");

    var bike_mark_pin = new H.map.Icon(base_url + "themes/backend/images/white_map_amb.png");
    var bike_red_mark_pin = new H.map.Icon(base_url + "themes/backend/images/red_bike_icon.png");
    var bike_green_mark_pin = new H.map.Icon(base_url + "themes/backend/images/select_bike_icon.png");
    var bike_orange_mark_pin = new H.map.Icon(base_url + "themes/backend/images/orange_bike_icon.png");


    var map_obj, $google_map_obj;
    var post_data_url = {
        url: base_url + 'amb/gpsapi',
        type: "POST",
        cache: false
    };
    ...
    function initmarker() {
        ...
        for (ambin = 0; ambin < ambids.length; ambin++) {
            ...
            /* if(parseFloat(amblatlong[ambids[ambin]].amb_type) == '2' || parseFloat(amblatlong[ambids[ambin]].amb_type) == 2){
             ...
             }else if(parseFloat(amblatlong[ambids[ambin]].amb_type) == '1' || parseFloat(amblatlong[ambids[ambin]].amb_type) == 1){
                // console.log("hi");
                 if(parseFloat(amblatlong[ambids[ambin]].amb_status) == '2' || parseFloat(amblatlong[ambids[ambin]].amb_status) == 2){

                     //map_mark_pin = bike_red_mark_pin;
                      map_mark_pin = bike_red_mark_pin;
                 }else if(parseFloat(amblatlong[ambids[ambin]].amb_status) == '6' || parseFloat(amblatlong[ambids[ambin]].amb_status) == 6){

                     map_mark_pin = bike_orange_mark_pin;
                 }else if(parseFloat(amblatlong[ambids[ambin]].amb_status) == '1' || parseFloat(amblatlong[ambids[ambin]].amb_status) == 1){

                     map_mark_pin = bike_green_mark_pin;
                 }else{
                      map_mark_pin = bike_orange_mark_pin;
                 }
                 //console.log(map_mark_pin);
             }else{ */
            if (parseFloat(amblatlong[ambids[ambin]].amb_status) == '2' || parseFloat(amblatlong[ambids[ambin]].amb_status) == 2) {
            ...
            var markerLatLng = {
                lat: parseFloat(amblatlong[ambids[ambin]].lat),
                lng: parseFloat(amblatlong[ambids[ambin]].lng)
            };
    ...
    function loadmap() {
        //var myLatLng = { lat: 34.138145, lng: 76.009479 };
        ...
        // addMarkersAndSetViewBounds($AllMap);
    ...
    function set_amb_direction($google_map_obj, $origin, $destination) {
        ...
        var $directionsDisplayAmb = new google.maps.DirectionsRenderer({

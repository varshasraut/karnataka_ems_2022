...
<div class="margin_auto width70 mt-4">
    <div class="dash_quality_marks text_align_center " >
		<?php if($clg_group == 'UG-COMPLIANCE'){
            ?>
             <strong>GOVERNMET COMPLIANCE CALL INFO</strong><br>
            <?php
        }else{
            ?>
            <strong>GRIEVANCE CALL INFO</strong><br>
            <?php
        }
        ?>
		...
		<table >
			<tr>
				<th style="text-align: center;">Total Complaint Raised <br>(This month)</th>
				<th style="text-align: center;">Total Closure <br> (This month)</th>
                <th style="text-align: center;">Total Pending  <br>(This month)</th>
                <th style="text-align: center;">Total Closure <br>(Today)</th>
                <th style="text-align: center;">Total Pending <br>(Today)</th>
           
            </tr>
			<tr>
				<td><?php echo $get_all_calls;?></td>
				<td><?php echo $get_all_closure_calls; ?></td>
				<td><?php echo $get_all_pending_calls; ?></td>
                <td><?php echo $get_today_closure_calls;?></td>
                <td><?php echo $get_today_pending_calls; ?></td>
			</tr>
		</table>
		<span><?php //echo $audit_details[0]->quality_score;?></span>
	</div>
</div>
<br><div id="calls_inc_list">
        ...
        <div class="width100" id="ero_dash">
            ...
            <div id="amb_filters">
                ...
                <div class="width100"> 
                    ...
                    <div class="width_16 float_left">   <h3 class="txt_clr2  float_left">Incident Calls</h3> </div>
                                                <?php if($clg_senior){?>
...
//                                        echo "selected";
//                                    }
                            ?>>Complaint Register No</option>
                            <option value="incident_id" 
                                    <?php
//                                    if ($filter == 'complaint_id') {
//                                        echo "selected";
//                                    }
...
//                                    }
                    ...
                    ?>">
                        <select name="complaint_type" class="dropdown_per_page width97" id="grievance_sub_filter_list" data-base="search" >
...
//                                    }
                            ...
                            <option value="government_letter" <?php
//                                    if ($amb_reg_id == $amb->amb_rto_register_no) {
//                                        echo 'selected';
//                                    }
                            ?>>Government Letter</option>
...
//                                    }
                    ...
                    <div id="inc_id_box1" class="width_16 float_left " style="<?php
                    if ($complaint_id != '') {
                        echo 'display:block;';
                    } else {
                        echo 'display:none;';
                    ...
                    ?>">
                        <input name="incident_id" tabindex="7.2" class=" form_input" placeholder="Incident ID" type="text"  data-errors="{filter_required:'Please select hospital from dropdown list'}" >
                    </div>
                    ...
                    <div class="float_left">
                        <input type="button" class="search_button float_left form-xhttp-request mt-0" name="" value="Search" data-href="{base_url}grievance" data-qr="output_position=content&amp;flt=true" />  
                        <input class="search_button click-xhttp-request float_right mt-0" name="" value="Reset Filters" data-href="{base_url}grievance" data-qr="output_position=content&amp;filters=reset" type="button">
                    </div>
                </div>
                            ...
                            if (count(@$inc_info) > 0) {
                                ...
                                foreach ($inc_info as $key => $inc) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $inc->gc_pre_inc_ref_id; ?></td>
  <!--                                      <td><?php echo $inc->gc_inc_ref_id; ?></td>-->
<!--                                         <td ><a href="{base_url}calls/single_record_view" class="onpage_popup" data-qr="output_position=popup_div&inc_ref_id=<?php echo $inc->gc_pre_inc_ref_id; ?>" style="color:#000;"><?php echo $inc->gc_pre_inc_ref_id; ?></a></td>-->
                                                        <td ><a href="{base_url}calls/single_record_view" class="onpage_popup" data-qr="output_position=popup_div&inc_ref_id=<?php echo $inc->gc_inc_ref_id; ?>" style="color:#000;"><?php echo $inc->gc_inc_ref_id; ?></a></td>
                                        <td><?php echo $inc->gc_caller_number; ?></td>
                                        <td><?php echo ucwords($inc->gc_caller_name); ?></td>
                                        <td><?php echo ucwords($inc->dst_name); ?></td>
                                        <td><?php  echo ucwords($type); ?></td>
                                        <td ><?php echo $inc->gc_e_complaint_no; ?></td>
                                        <td ><?php echo strtoupper($inc->gc_added_by); ?></td>

...
//    }
    ...
    public function index($generated = false) {
        ...
        if($user_group == 'UG-ERO-102' || $user_group == 'UG-ERO' || $user_group == 'UG-EROSupervisor' || $user_group == 'UG-REMOTE' || $user_group == 'UG-BIKE-ERO' || $user_group == 'UG-ERCTRAINING' ||$user_group == 'UG-ERO-104') {
       ...
       $data['all_purpose_of_104_calls'] = $this->call_model->get_all_child_purpose_of_104_calls();
        ...
        //$newdate =  date("Y-m-d H:i:s", (strtotime ('+1 hour' , strtotime ($aht)))) ;
...
//        }
       ...
       // die();
		...
		if($data['CallUniqueID'] == '' && $CallUniqueID == ''){
            ...
            if(!empty($inc_data)){
                $data['inc_ref_no'] = $inc_data[0]->inc_ref_id;
               
              
                if($service_type == 'avaya'){
                    $avaya_server_url = $this->avaya_server_url.'/AnswerCall';
                    ...
                    $avaya_data = json_encode($avaya_args);
                     file_put_contents('./logs/'.date("Y-m-d").'/'.$extension_no.'avaya_call_answer.log', $avaya_data.",\r\n", FILE_APPEND);
                    $avaya_resp =  cct_agent_answercall($avaya_server_url,$avaya_data);
                     file_put_contents('./logs/'.date("Y-m-d").'/'.$extension_no.'avaya_call_answer_responce.log', $avaya_resp["resp"].",\r\n", FILE_APPEND);
                    $avaya_resp = json_decode($avaya_resp["resp"]);
                    
                }else if($service_type == 'coral'){

                    $avaya_server_url = $this->avaya_server_url.'/autopickupphone';
           ...
           // die();
            ...
            //if($avaya_call->status == 'R'){
                ...
                if($service_type == 'avaya'){
                    ...
                    file_put_contents('./logs/'.date("Y-m-d").'/avaya_call_answer.log', $avaya_data.",\r\n", FILE_APPEND);
                    $avaya_resp =  cct_agent_answercall($avaya_server_url,$avaya_data);
                    file_put_contents('./logs/'.date("Y-m-d").'/avaya_call_answer_responce.log', $avaya_resp["resp"].",\r\n", FILE_APPEND);
                    
                    $avaya_resp = json_decode($avaya_resp["resp"]);
                    
                }else if($service_type == 'coral'){

                    $avaya_server_url = $this->coral_server_url.'/autopickupphone';
...
//            );
        ...
        if($parent_purpose == "EMG" || $parent_purpose == "PREGANCY_CARE_CALL" || $parent_purpose == "Child_CARE" || $parent_purpose == "DROP_BACK_CALL"){
          // echo "hi";
           // $this->output->add_to_position($this->load->view('frontend/calls/common_patient_info', $data, TRUE), 'call_common_info', TRUE);
            //$this->output->add_to_position('', 'content', TRUE);
        }else{
...
// Medical Details
    ...
    function get_incoming_calls() {
        ...
        if ($atend_user->clg_group == 'UG-ERCP') {
    ...
    public function all_listing_filter($generated = false) {
       ...
       // $data['page_no'] = $page_no;
        ...
        if($this->post['action'] == 'view'){
            ...
            $header = array('Date','Call Type','Incidence ID','Caller Name','District','Caller Mobile No','EMT NAME','EMT Mobile','ERO Name','Avaya ID','Audio File','Ambulance','Call Time','Incident status','Closure Status');
            fputcsv($fp, $header);
...
//        }
        ...
        // $this->output->add_to_position($this->load->view('frontend/calls/single_caller_history_view', $data, TRUE), 'popup_div', TRUE);
      //$this->output->add_to_popup($this->load->view('frontend/calls/single_caller_history_view', $data, TRUE), '1300', '560');
        $this->output->add_to_position($this->load->view('frontend/calls/single_caller_history_view', $data, TRUE), $position, TRUE);
    }

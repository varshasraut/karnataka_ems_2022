...
class Medication extends EMS_Controller {
    ...
    function medication_list() {
        ...
        $pgconf = array(
            'url' => base_url("medication/medication_list"),
        ...
        $this->output->add_to_position($this->load->view('frontend/medication/medication_list_view', $data, TRUE), $this->post['output_position'], TRUE);

        $this->output->add_to_position($this->load->view('frontend/medication/medication_filters_view', $data, TRUE), 'prescription_filters', TRUE);
    }
    function view_prescription(){
        ...
        $this->output->add_to_popup($this->load->view('frontend/medication/medication_view', $data, TRUE), '800', '800');
    
    }
    function save_medication(){
        $pre_id = $this->input->post('pre_id');
        $args = array('drug_intake' => json_encode( $this->input->post('dose')));
        
        $result = $this->prescription_model->update_prescription($pre_id,$args);
         if ($result) {
            $this->output->closepopup = 'yes';
            $this->output->status = 1;
            $this->output->message = "<div class='success'>" . "Medicines updated successfully" . "</div>";

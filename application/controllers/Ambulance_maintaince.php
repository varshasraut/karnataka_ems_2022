...
class Ambulance_maintaince extends EMS_Controller {
    ...
    function preventive_maintaince_registrartion() {
        ...
        if ($this->post['action_type'] == 'Update') {
            ...
            $data['action_type'] = "Update Schedule Service";
            $data['type'] = "Update";
            $data['update'] = 'True';
            ...
            $data['action_type'] = "Add Schedule Service";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/preventive_maintaince_register_view', $data, TRUE), '1600', '1000');
        }
    }
    ...
    function approve_preventive_maintaince() {
        ...
        if ($this->post['action_type'] == 'Update') {
            ...
            $data['type'] = "Approve";
            $data['action_type'] = "Approve Schedule Service";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/preventive_maintaince_register_view', $data, TRUE), '1600', '1000');
        }elseif($this->post['action_type'] == 'Rerequest') {
            $data['type'] = "Rerequest";
            $data['action_type'] = "Re-request Schedule Service";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/preventive_maintaince_register_view', $data, TRUE), '1600', '1000');
        }else{
            $data['action_type'] = "Add Schedule Service";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/preventive_maintaince_register_view', $data, TRUE), '1600', '1000');
        ...
        function update_approve_preventive_maintaince() {
            ...
            if($app_data['mt_approval']=="1"){
                ...
                // $data = array(
                //     'vehicle_no' => $this->input->post('maintaince_ambulance')
                // );
                // $update = $this->amb_model->logout_emt_pilot($data);
        ...
        if($app_data['mt_approval']=="1"){
            ...
            $amb_update_summary = array(
                ...
                'off_road_status' => "Ambulance Schedule Service off road",
        ...
        //    var_dump($data);die;
            ...
            if ($register_result) {
                ...
                $this->output->message = "<div class='success'>Schedule Service Approved Successfully!</div>";
...
//              $approval = 'Approved';
    ...
    function preventive_maintaince_save() {
        ...
        $maintaince_data['mt_stnd_remark'] = "Schedule Service";
...
//   var_dump($rsm_config);die();   
    ...
    //         'mt_offroad_reason' => 'Schedule Service',
              ...
              //  var_dump($register_result_clg);
                if($register_result_clg){
                    ...
                    foreach($register_result_clg as $register){
                        ...
                        $body ="<H2>Reason	Vehicle is due for Schedule Service (Schedule Service)<H2><br><table><tr><td>Ambulance Registration No:</td><td>".$this->input->post('maintaince_ambulance')."</td></tr>
...
<tr><td>Schedule Service Type:</td><td>Schedule Service</td></tr>
...
<tr><td>Expected Date & Time of Completion Schedule Servicing Work</td><td>".$maintaince_data['mt_ex_onroad_datetime']."</td></tr></table><br>Thanks & Regards<br>ADM, JEAS - MEMS";
                       ...
                       // $email = "chaitali.fartade@mulikainfotech.com";
                        $subject = "Ambulance On Schedule Service";
        ...
        if ($register_result) {
           ...
           // $update = $this->amb_model->update_amb($data);
            ...
            $this->output->message = "<div class='success'>Schedule Service Registered Successfully!</div>";
    ...
    function update_preventive_maintaince() {
        ...
        if(!empty($_FILES['amb_invoice']['name'])){
            foreach ($_FILES['amb_invoice']['name'] as $key => $image) {
                ...
                $_FILES['invoice']['name']= $_FILES['amb_invoice']['name'][$key];
                $_FILES['invoice']['type']= $_FILES['amb_invoice']['type'][$key];
                $_FILES['invoice']['tmp_name']= $_FILES['amb_invoice']['tmp_name'][$key];
                $_FILES['invoice']['error']= $_FILES['amb_invoice']['error'][$key];
                $_FILES['invoice']['size']= $_FILES['amb_invoice']['size'][$key];

                $_FILES['invoice']['name'] = time().'_'.$this->sanitize_file_name($_FILES['invoice']['name']);
               // var_dump($_FILES['amb_photo']['name']);
                //die();
                ...
                $media_args['media_name'] = $_FILES['invoice']['name'];
                $media_args['user_id'] = $maintaince_data['mt_id'];
    ...
    function preventive_maintaince_view() {
        ...
        $data['action_type'] = 'View Schedule Service';
...
//var_dump($this->input->post()); die;
    ...
    { 
        ...
        if ($this->post['action_type'] == 'Update') {
            ...
            $data['action_type'] = "Update General Offroad/Standby Maintenance";
            $data['type'] = "Update";
            $data['update'] = 'True';
            ...
            $data['type'] = "Approve";
            $data['action_type'] = "Approve General Offroad/Standby Maintenance";

            $this->output->add_to_popup($this->load->view('frontend/maintaince/onroad_offroad_maintaince_register_view', $data, TRUE), '1200', '500');
        }elseif($this->post['action_type'] == 'Rerequest') {
            $data['type'] = "Rerequest";
            $data['action_type'] = "Re-request General Offroad/Standby Maintenance";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/onroad_offroad_maintaince_register_view', $data, TRUE), '1200', '500');
        }
       /* if ($this->post['mt_id'] != '') {
...
//        die();
               ...
               $pilot_login_status = $this->amb_model->get_amb_login_status(array('amb_rto_register_no'=>$this->input->post('maintaince_ambulance'),'type'=>'P','status'=>'1'));
                ...
                if(!empty($pilot_login_status)){
                    ...
                    $update = $this->amb_model->update_amb_login_status($pilot_login_status[0]->id,$pilot_status);  
                        ...
                        $data_insert = array('logout_remark'=>'Ambulance Offroad-Onroad Pilot',
                            'logout_ref_id'=> $pilot_login_status[0]->clg_id,
                            'added_by'=>$this->clg->clg_ref_id,
                            'added_date'=>date('Y-m-d H:i:s'));
                ...
                if(!empty($driver_login_status)){
                    ...
                    $data_insert = array('logout_remark'=>'Ambulance Offroad-Onroad Driver',
                            'logout_ref_id'=> $driver_login_status[0]->clg_id,
                            'added_by'=>$this->clg->clg_ref_id,
                            'added_date'=>date('Y-m-d H:i:s'));
    ...
    function onroad_offroad_maintaince_registrartion() {
        ...
        if ($this->post['action_type'] == 'Update') {
            ...
            $data['action_type'] = "Rerequest General Offroad/Standby Maintenance";
            $data['type'] = "Rerequest";
            $data['Rerequest'] = 'True';
            ...
            $data['action_type'] = "Add General Offroad/Standby Maintenance";
            $state_id = $this->clg->clg_state_id;
            $data['state_id'] = $state_id;
            $this->output->add_to_popup($this->load->view('frontend/maintaince/onroad_offroad_maintaince_register_view', $data, TRUE), '1200', '500');
        }
    ...
    function onroad_offroad_maintaince_save() {
        ...
        $main_data = array('mt_state_id' => $this->input->post('maintaince_state'),
           ...
           // 'mt_offroad_datetime' => date('Y-m-d H:i:s', strtotime($maintaince_data['mt_offroad_datetime'])),
            // 'mt_ex_onroad_datetime' =>$this->input->post('ex_onroad_datetime'),
            'mt_offroad_reason'       => $maintaince_data['mt_offroad_reason'],
             'amb_type' => $this->input->post('amb_type'),
            'mt_make' => $this->input->post('ambt_make'),
            'mt_module' => $this->input->post('ambt_model'),
            'added_by' => $this->clg->clg_ref_id,
    ...
    function onroad_offroad_maintaince_view() {
        ...
        $data = array();
        $data['action_type'] = 'View General Offroad/Standby Maintenance';
   ...
   function offraod_preventive_maintaince_cron() {
    ...
    function man_power(){
       ...
       // var_dump($this->fdata['from_date']); die;
        ...
        //$data['thirdparty'] = $this->clg->thirdparty;
        $data['total_count'] = $this->ambmain_model->get_ambulance_manpower_data($data);
        ...
        $data['maintance_data'] = $this->ambmain_model->get_ambulance_manpower_data($data, $offset, $limit);
       // var_dump($data['maintance_data']);
       // die;
        /////////////////////////
        ...
        $pgconf = array(
            'url' => base_url("ambulance_maintaince/man_power"),
            'total_rows' => $data['total_count'],
            'per_page' => $limit,
            'cur_page' => $page_no,
            'attributes' => array('class' => 'click-xhttp-request',
        ...
        $this->output->add_to_position($this->load->view('frontend/maintaince/man_power_list_view', $data, true), $this->post['output_position'], true);
    }
    function man_power_add(){
        ...
        if ($this->post['mt_id'] != '') {
            ...
            $data['mt_id'] = $this->post['mt_id'];
            $data['mt_type'] = 'manpower';
            $data['preventive'] = $this->ambmain_model->get_ambulance_manpower_data($data);
        }


        if ($this->post['action_type'] == 'Update') {

            $data['action_type'] = "Update Manpower";
            $data['type'] = "Update";
            $data['update'] = 'True';

            $this->output->add_to_popup($this->load->view('frontend/maintaince/man_power_register_view', $data, TRUE), '1600', '1000');
        }else if ($this->post['action_type'] == 'Approve') {

            $data['action_type'] = "Approve Manpower";
            $data['type'] = "Approve";
            $data['update'] = 'True';
            ...
            $state_id = $this->clg->clg_state_id;
            $data['state_id'] = $state_id;
            $data['action_type'] = "Add Manpower";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/man_power_register_view', $data, TRUE), '1600', '1000');
        }
    }
    function manpower_save(){
        $maintaince_data = $this->post['maintaince'];
        $maintaince_data['mt_stnd_remark'] = "ManPower";
        $exp_onroad = date('Y-m-d H:i:s', strtotime($maintaince_data['mt_ontime_onroad_date']));
        $Offroad = date('Y-m-d H:i:s', strtotime($maintaince_data['mt_offroad_date']));
        ...
        $state = $this->input->post('maintaince_state');
            ...
            $prev= generate_maintaince_id('ems_manpower_id');
            $prev_id = "MP-".$prev;
        ...
        //var_dump($district_id);die();
        $main_data = array(
            'mt_manpower_id'=>$prev_id,
            'mt_state_id' => "MP",
            'mt_district_id' => $district_id,
            'mt_amb_no' => $this->input->post('maintaince_ambulance'),
            'mt_base_loc' => $this->input->post('base_location'),

            'mt_ontime_onroad_date' => $exp_onroad,
            'mt_offroad_date' => $Offroad,
            ...
            'mt_previos_odometer' => $this->input->post('previous_odometer'),
            'mt_make' => $this->input->post('ambt_make'),
            'mt_manpower_issue_for' => $maintaince_data['mt_manpower_issue_for'],
            'added_by' => $this->clg->clg_ref_id,
            'added_date' => date('Y-m-d H:i:s'),
            'modify_by' => $this->clg->clg_ref_id,
            'modify_date' => date('Y-m-d H:i:s'),
        ...
        //$args = array_merge($this->post['maintaince'], $main_data);
            ...
            // var_dump($main_data);die();
        $register_result = $this->ambmain_model->insert_manpower_maintance($main_data);
       ...
       $amb_record_data = array(
        ...
        if (!empty($maintaince_data['mt_remark'])) {
            ...
            if ($register_result) {
                ...
                $this->output->message = "<div class='success'>Manpower saved Successfully!</div>";
    ...
    function approve_manpower(){
            ...
            $register_result = $this->ambmain_model->update_manpower($app_data);
            ...
            // $update = $this->amb_model->logout_emt_pilot($data);
                ...
                $this->output->message = "<div class='success'>Manpower $approval Successfully!</div>";
    ...
    function update_manpower(){
            $app_data = $this->input->post('app');
            $app_data = $this->input->post('maintaince');
            $Approved_by = $this->clg->clg_ref_id;
            $approved_date = date('Y-m-d H:i:s');
            $main_onroad = date('Y-m-d H:i:s', strtotime($app_data['mt_ex_onroad_datetime']));
            ...
            'mt_id' => $app_data['mt_id'],
            'mt_isupdated ' => '1',
            'mt_on_remark' => $app_data['mt_on_remark'],
            'mt_ex_onroad_datetime' => $main_onroad,
            'mt_end_odometer' => $this->input->post('mt_end_odometer'),
            'mt_ambulance_status' => $approval,
            'modify_by' => $this->clg->clg_ref_id,
            'modify_date' => date('Y-m-d H:i:s'),
        ...
        if (!empty($maintaince_data['mt_remark'])) {
            ...
            if ($register_result) {
                ...
                $data = array(
                        ...
                        'amb_rto_register_no' => $amb_rto_register_no,
                        'amb_status' => '1',
                        // 'ambis_backup' => '0',
                    );
                    // var_dump($data);die();
                ...
                $this->output->message = "<div class='success'>Manpower Approved Successfully!</div>";
    ...
    function manpower_view() {


        $data = array();


        $data['action_type'] = 'View Manpower';

        $data['mt_id'] = $ref_id = $this->post['mt_id'];
        $data['preventive'] = $this->ambmain_model->get_ambulance_manpower_data($data);
        $data['mt_type'] ='manpower';

        $this->output->add_to_popup($this->load->view('frontend/maintaince/man_power_view', $data, TRUE), '1600', '1000');
    }
    
    function scrape_vahicle(){
       ...
       // var_dump($this->fdata['from_date']); die;
        ...
        //$data['thirdparty'] = $this->clg->thirdparty;
        $data['total_count'] = $this->ambmain_model->get_ambulance_scrape_vahicle_data($data);
        ...
        $data['maintance_data'] = $this->ambmain_model->get_ambulance_scrape_vahicle_data($data, $offset, $limit);
       // var_dump($data['maintance_data']);
       // die;
        /////////////////////////
        ...
        $pgconf = array(
            'url' => base_url("ambulance_maintaince/scrape_vahicle"),
            'total_rows' => $data['total_count'],
            'per_page' => $limit,
            'cur_page' => $page_no,
            'attributes' => array('class' => 'click-xhttp-request',
        ...
        $this->output->add_to_position($this->load->view('frontend/maintaince/scrabe_vahicle_list_view', $data, true), $this->post['output_position'], true);
    }
    function scrape_vahicle_add(){
        ...
        if ($this->post['mt_id'] != '') {
            ...
            $data['mt_id'] = $this->post['mt_id'];
            $data['mt_type'] = 'scrape_vahicle';
            $data['preventive'] = $this->ambmain_model->get_ambulance_scrape_vahicle_data($data);
        }


        if ($this->post['action_type'] == 'Update') {

            $data['action_type'] = "Update Scrap Ambulance";
            $data['type'] = "Update";
            $data['update'] = 'True';

            $this->output->add_to_popup($this->load->view('frontend/maintaince/scabe_vahicle_register_view', $data, TRUE), '1600', '1000');
        }else if ($this->post['action_type'] == 'Approve') {

            $data['action_type'] = "Approve Scrap Ambulance";
            $data['type'] = "Approve";
            $data['update'] = 'True';
            ...
            $state_id = $this->clg->clg_state_id;
            $data['state_id'] = $state_id;
            $data['action_type'] = "Add Scrap Ambulance";
            $this->output->add_to_popup($this->load->view('frontend/maintaince/scabe_vahicle_register_view', $data, TRUE), '1600', '1000');
        }
    }
    function save_scrape_vahicle(){
        $maintaince_data = $this->post['maintaince'];
        $maintaince_data['mt_stnd_remark'] = "Scrape Ambulance";
        ...
        $state = $this->input->post('maintaince_state');
            ...
            $prev= generate_maintaince_id('ems_scrape_vahicles_id');
        ...
        $sp_date = date('Y-m-d H:i:s', strtotime($maintaince_data['mt_scrap_added_date']));
        $mt_ex_onroad_datetime = date('Y-m-d H:i:s', strtotime($maintaince_data['mt_ex_onroad_datetime']));

        //var_dump($district_id);die();
        $main_data = array(
            'mt_scrape_vahicle '=>$prev_id,
            'mt_state_id' => "MP",
            'mt_district_id' => $district_id,
            'mt_amb_no' => $this->input->post('maintaince_ambulance'),
            'mt_base_loc' => $this->input->post('base_location'),
            'mt_in_odometer' => $this->input->post('in_odometer'),
            'mt_scrap_added_date' => $sp_date,
            'mt_ex_onroad_datetime' => $mt_ex_onroad_datetime,
            'mt_previos_odometer' => $this->input->post('previous_odometer'),
            //'mt_total_km' => $this->input->post('odometer_diff'),
             'amb_type' => $this->input->post('amb_type'),
            'mt_make' => $this->input->post('ambt_make'),
            //'mt_inactive' => $maintaince_data['mt_inactive'],
            'added_by' => $this->clg->clg_ref_id,
            'added_date' => date('Y-m-d H:i:s'),
            'modify_by' => $this->clg->clg_ref_id,
            'modify_date' => date('Y-m-d H:i:s'),
        ...
        $register_result = $this->ambmain_model->insert_scrape_vahicle_maintance($main_data);
    ...
    //         'mt_isdeleted' => '0',
    //         'mt_type' => 'scrape_vahicle',
    //         'mt_offroad_reason' => 'Scrape Vahicle',
    //         'mt_remark' => $maintaince_data['mt_remark'],
    //         'mt_base_month' => $this->post['base_month'],
    //         'mt_ambulance_status' => 'Pending Approval'
       ...
       $amb_record_data = array(
        ...
        if (!empty($maintaince_data['mt_remark'])) {
            ...
            if ($register_result) {
                ...
                $this->output->message = "<div class='success'>Scrap Ambulance saved Successfully!</div>";
    ...
    function approve_scrape_vahicle(){
            $app_data = $this->input->post('app');
            ...
            $register_result = $this->ambmain_model->update_scrape_vahicle($app_data);
            ...
            if ($register_result) {
                ...
                $this->output->message = "<div class='success'>Scrap Ambulance Approved Successfully!</div>";
    ...
    function update_scrape_vahicle(){
            $app_data = $this->input->post('app');
            ...
            'start_odmeter' => $this->input->post('previous_odometer'),
            'end_odmeter' => $this->input->post('mt_end_odometer'),
            'odometer_type' => 'Scrape Vahicle Update',
            'total_km' => $total_km,
            'timestamp' => date('Y-m-d H:i:s'));

        if (!empty($maintaince_data['mt_remark'])) {
            $amb_record_data['other_remark'] = $maintaince_data['mt_on_remark'];
        }
                ...
                $this->output->message = "<div class='success'>Scrap Ambulance Updated Successfully!</div>";
    ...
    function scrape_vahicle_view() {


        $data = array();


        $data['action_type'] = 'View Scrap Vehicle';

        $data['mt_id'] = $ref_id = $this->post['mt_id'];
        $data['preventive'] = $this->ambmain_model->get_ambulance_scrape_vahicle_data($data);
        $data['mt_type'] ='scrape_vahicle';

        $this->output->add_to_popup($this->load->view('frontend/maintaince/scrape_vahicle_view', $data, TRUE), '1600', '1000');
    }

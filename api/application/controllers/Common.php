...
class Common extends REST_Controller {
    ...
    public function providerimpression_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            $incRec = $this->CommonModel->getCalltypeAsPerInc($incidentID);
            if(!empty($incRec)){
                if($incRec[0]['inc_type'] == 'DROP_BACK' || $incRec[0]['inc_type'] == 'PICK_UP'){
                    $proImp = $this->CommonModel->PatientProviderImpression($gender,$incRec[0]['inc_type']);
                }else{
                    $proImp = $this->CommonModel->PatientProviderImpression($gender,$incRec[0]['inc_type']);
                }
            }
        ...
        if((isset($_COOKIE['cookie']))){
            ...
            foreach($paymentmode as $paymentmode1){
                $data = array(
                    ...
                    'code' => $paymentmode1['payment_code']
                );
        ...
        if((isset($_COOKIE['cookie']))){
            ...
            if(!empty($getAmbType)){
                if($getAmbType[0]['ar_name'] == 'Rural'){
                    $time = (int) "25"; //Only accept minutes
                }else{
                    $time = (int) "18"; //Only accept minutes minutes
                }
            }
            $this->response([
                'data' => ([
    ...
    public function medicaladvicegivenby_post(){
        $data = $this->CommonModel->getmedicaladvicegivenby();
        $givenby = array();
        foreach($data as $data1){
            $data = array(
                'id' => (int) $data1['clg_id'],
                'name' => $data1['clg_first_name'].' '.$data1['clg_mid_name'].' '.$data1['clg_last_name']
            );
            array_push($givenby,$data);
        }
        $this->response([
            'data' => $givenby,
            'error' => null
    ...
    public function isPatientAvaReason_post(){
        if((isset($_COOKIE['cookie']))){
            $data = $this->CommonModel->getisPatientAvaReason();
            $reason = array();
            foreach($data as $data1){
                $reason1 = array(
                    'id' => $data1['id'],
    ...
    public function abandonedremark_post(){
        $type = $this->post('type');
        if((isset($_COOKIE['cookie']))){
            $data = $this->CommonModel->getabandonedremark($type);
            $this->response([
                'data' => $data,
                'error' => null
            ],REST_Controller::HTTP_OK);
    ...
    public function bloodgrp_post(){
        $rec = $this->CommonModel->getbloodgrp();
        $blood = array();
        foreach($rec as $rec1){
            $blood1 = array(
                'id' => (int) $rec1['bldgrp_id'],
                'name' => $rec1['bldgrp_name']
            );
            array_push($blood,$blood1);
        }
        $this->response([
            'data' => $blood,
            'error' => null
    ...
    public function vehiclechecklist_post(){
        $equipmentChecked = $this->post('equipmentChecked');
        $consumableChecked = $this->post('consumableChecked');
        $oxygenCylinderChecked = $this->post('oxygenCylinderChecked');
        $type = $this->post('type');
        $vehicle = $this->post('vehicleNumber');
        $veh = explode(' ',$vehicle);
        $vehicleNumber = implode('-',$veh);
        ...
        $chklist['veh_chklist_eqp_checked'] = $equipmentChecked;
        $chklist['veh_chklist_consumable_checked'] = $consumableChecked;
        $chklist['veh_chklist_oxy_cylin_checked'] = $oxygenCylinderChecked;
        $chklist['veh_chklist_type'] = $type;
        $chklist['veh_chklist_vehicle_no'] = $vehicleNumber;
        $chklist['veh_chklist_operated_by'] = $dp_operated_by1;
        $chklist['veh_chklist_added_date'] = date('Y-m-d h:i:s');
        $this->CommonModel->insertVehicleChecklist($chklist);
        $this->response([
            'data' => [
                "code" => 1,
    ...
    public function pvthospamount_post(){
        $incidentId = $this->post('incidentId');
        $type = $this->post('type');
        $data1 = $this->User->get_pvt_hosp_amount($incidentId);
        if(!empty($data1)){
            $pvtHospAmt = $data1[0]['pr_total_amount'];
        }else{
            $pvtHospAmt = '';
        }
        $this->response([
            'data' => ([
                'pvthospamount' => $pvtHospAmt,
            ]),
            'error' => null
        ],REST_Controller::HTTP_OK);
        
    }
    public function pvthosppaymentmode_post(){
        $data = $this->CommonModel->get_pvt_hosp_payment_mode();
        // print_r($data);
        $pvthosp = array();
        foreach($data as $data1){
            $pvthosp1 = array(
                "id" => (int) $data1['id'],
                "name" => $data1['payment_name']
            );
            array_push($pvthosp,$pvthosp1);
        }
        $this->response([
            'data' => $pvthosp,
            'error' => null
    ...
    public function ododiffaspercalltype_post(){
        $type = $this->post('type');
        $incidentId = $this->post('incidentId');
        $vehicle = $this->post('vehicleNumber');
        //$driverParameter = $this->post('driverParameter');
        $veh = explode(' ',$vehicle);
        $vehicleNumber = implode('-',$veh);
        $rec = $this->CommonModel->getcalltype($incidentId);
        if(!empty($rec)){
            if($rec[0]['inc_type'] == 'IN_HO_P_TR'){
                $difference = 1000;
            }else{
                $difference = 500;
            }
        ...
        $this->response([
            'data' => ([
                'odometerDifference' => (int) $difference
            ]),
            'error' => null
        ],REST_Controller::HTTP_OK);
    }
    public function getlastodologinlogout_post(){
        $loginLogoutType = $this->post('loginLogoutType');
        $vehicle = $this->post('vehicleNumber');
        $veh = explode(' ',$vehicle);
        $vehicleNumber = implode('-',$veh);
        $rec = $this->CommonModel->getlastodologinlogout($loginLogoutType,$vehicleNumber);
        if(!empty($rec)){
            $loginLogoutOdo = (int) $rec[0]['login_logout_odo_odometer'];
        }else{
            $loginLogoutOdo = (int) 0;
        }
        $this->response([
            'data' => ([
                'loginLogoutOdo' => $loginLogoutOdo
            ]),
            'error' => null
        ],REST_Controller::HTTP_OK);
    }
    public function fuelfillingprevodoandlastupdodo_post(){
        $vehicle = $this->post('vehicleNumber');
        $veh = explode(' ',$vehicle);
        $vehicleNumber = implode('-',$veh);
        $prevodo = $this->CommonModel->fuelfillingprevodo($vehicleNumber);
        $lastupdodo = $this->CommonModel->fuelfillinglastupdodo($vehicleNumber);
        if(!empty($prevodo)){
            $previousOdo = (int) $prevodo[0]->ff_current_odometer;
        }else{
            $previousOdo = (int) 0;
        }
        if(!empty($lastupdodo)){
            $lastupdateodo = (int) $lastupdodo[0]->end_odmeter;
        }else{
            $lastupdateodo = (int) 0;
        }
        $this->response([
            'data' => ([
                ...
                'lastupdateodo' => $lastupdateodo
            ]),
            'error' => null
        ],REST_Controller::HTTP_OK);
    }
    public function getshift_post(){
        $data = $this->CommonModel->get_shift();
        // print_r($data);
        $shift = array();
        foreach($data as $data1){
            $shift1 = array(
                "id" => (int) $data1['shift_id'],
                "name" => $data1['shift_name']
            );
            array_push($shift,$shift1);
        }
        $this->response([
            'data' => $shift,
            'error' => null

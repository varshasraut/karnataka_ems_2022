...
// Authentication
class Supervisory_Device extends REST_Controller {

    public function __construct() { 
        parent::__construct();
        // Load the user model
        $this->load->model('supervisory_app_api_models/Supervisory_Device_model');
    ...
    public function index_post(){
        ...
        // print_r($data);die;
        $version = $this->Supervisory_Device_model->getCurrentversion($osName);
        if(isset($version[0])){
        ...
        if($uniqueId==0)
	    {
            ...
            {
                $currentId = $this->Supervisory_Device_model->insertDeviceVersion($data);
                $this->response([
                    'data' => ([
            ...
            {
                $checkDeviceId = $this->Supervisory_Device_model->updateDeviceVersion($data,$uniqueId);
                if($checkDeviceId == 1){
                    $this->response([
                        'data' => null,
                        'error' => ([
        ...
        $this->response([
            ...
            'error' => ([
                'code' => 4,
                'message' => 'Os Name '.$osName.' not found in our records'
            ])
        ],REST_Controller::HTTP_OK);
    }

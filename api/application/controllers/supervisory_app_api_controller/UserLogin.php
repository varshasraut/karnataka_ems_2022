...
class UserLogin extends REST_Controller {
    ...
    public function index_post(){
        ...
        if(isset($data['username']) && isset($data['password'])){
            ...
            $loginData = $this->Login_model->checkLoginUser($data);
            // print_r($loginData);
            $loginCheck = $this->Login_model->loginCheck($data);
            // echo $loginSecretKey;
            if(is_array($loginCheck)){
                if(empty($loginData)){
                    ...
                    $loginSuccess = $this->Login_model->loginSuccess($login);
                    if($loginSuccess == 1){
                        $this->response([
                            'data' => ([
                                'code' => 1,
                    ...
                    if(!empty($loginData) && ($data['device_id']==$loginData[0]->device_id)){ 
                        ...
                        $login['device_id'] = $data['device_id'];
                        $login['login_status'] = '1';
                        $loginData = $this->Login_model->logoutPreDevice($data);
                        $loginSuccess = $this->Login_model->loginSuccess($login);

...
class PatientEditmedicine extends REST_Controller {
    ...
    public function index_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            if(!empty($patientId)){
                $args = "MCA";
                $medicine = $this->user->getmedicinename($patientId,$args,$ambulanceNo);
            }else{
                $args = "Req";
                $medicine = $this->user->getmedicinename($requestId,$args,$ambulanceNo);
            }
            if(!empty($medicine)){
                $this->response([
                    'data' => $medicine,
    ...
    public function medicinenonunit_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            if(!empty($patientId)){
                $args = "MNCA";
                $medicine = $this->user->getNonUnitMedicineName($patientId,$args,$ambulanceNo);
            }else{
                $args = "Req";
                $medicine = $this->user->getNonUnitMedicineName($requestId,$args,$ambulanceNo);
            }
            if(!empty($medicine)){
                $this->response([
                    'data' => $medicine,

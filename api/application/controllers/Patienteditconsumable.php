...
class Patienteditconsumable extends REST_Controller {
    ...
    public function index_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            if(!empty($patientId)){
                $args = "CA";
                $consumable = $this->user->ConsumableList($patientId,$args,$ambulanceNo);
            }else{
                $args = "Req";
                $consumable = $this->user->ConsumableList($requestId,$args,$ambulanceNo);
            }
            if(!empty($consumable)){
                $this->response([
                    'data' => $consumable,
    ...
    // }
     public function cosumablenonunit_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            if(!empty($patientId)){
                $args = "CA";
                $consumable = $this->user->ConsumableNonUnitList($patientId,$args,$ambulanceNo);
            }else{
                $args = "Req";
                $consumable = $this->user->ConsumableNonUnitList($requestId,$args,$ambulanceNo);
            }
            if(!empty($consumable)){
                $this->response([
                    'data' => $consumable,

...
class Patientadd extends REST_Controller {
    public function __construct() { 
        ...
        $this->load->helper(array('cookie', 'url','common_helper'));
    ...
    public function index_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            $ayushmanId = $this->post('ayushmanId');
            $pcf = $this->post('pcf');
            ...
            if($chklogindata == 1 || (empty($deviceIdLogin))){
                ...
                $getBaseLoc = $this->user->getBaseLoc($incidentID);
                if(!empty($getBaseLoc)){
                    $base_loc_id = $getBaseLoc[0]['base_location_id'];
                    $base_loc_name = $getBaseLoc[0]['base_location_name'];
                }
                $dp_operated_by = array();
                $clg_id_for_new_ptn = array();
                if(empty($logindata)){
                    $this->response([
                        'data' => null,
                        'error' => ([
                    ...
                    foreach($logindata as $logindata1){
                        if(count($logindata)==1){
                            if($logindata1['login_type'] == 'P'){
                                ...
                                $emso_id1 = $logindata1['clg_id'];
                                array_push($dp_operated_by,$emso_id);
                                array_push($clg_id_for_new_ptn,$emso_id1);
                            }else{
                                $pilot_id = $logindata1['clg_ref_id'];
                                $pilot_id1 = $logindata1['clg_id'];
                                array_push($dp_operated_by,$pilot_id);
                                array_push($clg_id_for_new_ptn,$pilot_id1);
                            }
                        }else{
                            if($logindata1['login_type'] == 'P'){
                                $emso_id = $logindata1['clg_ref_id'];
                ...
                // print_r($clg_id_for_new_ptn);die;
            
                if(count($clg_id_for_new_ptn) == 2){
                    $clg_id_for_new_ptn1 = $clg_id_for_new_ptn[0];
                }else{
                    if(!empty($clg_id_for_new_ptn)){
                        $clg_id_for_new_ptn1 = $clg_id_for_new_ptn[0];
                    }
                }
                $last_pat_id = generate_ptn_id($clg_id_for_new_ptn1);
                // echo $last_pat_id;die;
                ...
                //print_r($data);
                            ...
                            $incPat['ptn_id'] = $PatientId1[0]->ptn_id;
                            $incPat['id'] = $primary_key;
                            $lastIncPtnId = $this->user->insertIncidencePatient($incPat);
                            $patientInfo = $this->user->getPatientInfo($PatientId1[0]->ptn_id);
                            ...
                            $epcr['system_type'] = $incidentData[0]['inc_system_type'];
                            $epcr['base_location_id'] = $base_loc_id;
                            $epcr['base_location_name'] = $base_loc_name;
                            // print_r($logindata);
                            if(empty($logindata)){
                                $this->response([
                                    'data' => null,

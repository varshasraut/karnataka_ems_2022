...
class Fuelfilling extends REST_Controller {
    ...
    public function index_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            $ffCaseTypeRemark = $this->post('ffCaseTypeRemark');
            ...
            $ffFuelPreviousOdometer = $this->post('ffFuelPreviousOdometer');
            $ffPreviousOdometer = $this->post('ffPreviousOdometer');
            $currentOdometer = $this->post('currentOdometer');
            $fuelDateTime = $this->post('fuelDateTime');
            $distanceTravelled = $this->post('distanceTravelled');
            $kmpl = $this->post('kmpl');
            $fuelFillingCase = $this->post('fuelFillingCase');
            $fuelOtherStation = $this->post('fuelOtherStation');
            $fuelOtherStationAdd = $this->post('fuelOtherStationAdd');
            $ffTypeRemark = $this->post('ffTypeRemark');
            $ffRate = $this->post('ffRate');
            $ffBill = $this->post('ffBill');
            ...
            $path = FCPATH;
            $str = str_replace("api/", "", $path);
            $extractpath = $str.'assets/images/nozzle_slip/';
            // echo $extractpath;die;
            $date = date("Y_m_d");
                ...
                $added_by = array();
                    for($i=0;$i<count($userLoginArr);$i++){
                        if($userLoginArr[$i] && $userLoginArr1[$i] == 'D'){
                            ...
                            $userName = $this->user->getClgRefName($userLoginArr[$i]);
                            $data['ff_pilot_id'] = $user2;//$userLoginArr[$i];
                            $data['ff_pilot_name'] = $userName[0]->clg_first_name.' '.$userName[0]->clg_last_name;
                            array_push($added_by,$data['ff_pilot_id']);
                        }else if($userLoginArr[$i] && $userLoginArr1[$i] == 'P'){
                            $user1 = $this->user->getClgRefid($userLoginArr[$i]);
                            $userName = $this->user->getClgRefName($userLoginArr[$i]);
                            $data['ff_emso_id'] = $user1;//$userLoginArr[$i];
                            $data['ff_emso_name'] = $userName[0]->clg_first_name.' '.$userName[0]->clg_last_name;
                            array_push($added_by,$data['ff_emso_id']);
                        }
                    }
                    $data['ff_added_by'] = implode(',',$added_by);
                    $data['ff_modify_by'] = implode(',',$added_by);
                    array_push($added_user,implode(',',$added_by));
            }else if($type == 1){
                $loginUser = $this->user->getClgRefid($logindata['id']);
                $userName = $this->user->getClgRefName($logindata['id']);
                $data['ff_pilot_id'] = $loginUser;//$logindata['id'];
                $data['ff_pilot_name'] = $data['ff_pilot_name'] = $userName[0]->clg_first_name.' '.$userName[0]->clg_last_name;
                $data['ff_added_by'] = $data['ff_pilot_id'];
                $data2['ff_modify_by'] = $data['ff_pilot_id'];
                array_push($added_user,$data['ff_pilot_id']);
            }else{
                $loginUser = $this->user->getClgRefid($logindata['id']);
                $userName = $this->user->getClgRefName($logindata['id']);
                $data['ff_emso_id'] = $loginUser;//$logindata['id'];
                $data['ff_emso_name'] = $data['ff_pilot_name'] = $userName[0]->clg_first_name.' '.$userName[0]->clg_last_name;
                $data['ff_added_by'] = $data['ff_emso_id'];
                $data['ff_modify_by'] = $data['ff_emso_id'];
                array_push($added_user,$data['ff_emso_id']);
            }
            if(empty($requestId)){
                ...
                if($logindata == 1){
                    ...
                    if(!empty($amb) && !empty($station)){
                        ...
                        $data['ff_base_location'] = $amb[0]['basename'];
                        // $data['ff_fuel_previous_odometer'] = $ffFuelPreviousOdometer;
                        $data['ff_previous_odometer'] = $ffPreviousOdometer;
                        // $data['ff_in_odometer'] = $currentOdometer;
                        $data['ff_current_odometer'] = $ffPreviousOdometer;
                        $data['ff_fuel_mobile_no'] = $station ? $station[0]['f_station_mobile_no'] : "";
                        $data['ff_fuel_address'] = $station ? $station[0]['f_address'] : "";
                        $data['ff_added_date'] = date('Y-m-d H:i:s');
                        $data['ff_modify_date'] = date('Y-m-d H:i:s');
                        $data['ff_is_deleted'] = '0';
                        ...
                        $data['ff_other_fuel_station'] = $fuelOtherStation;
                        if(!empty($fuelOtherStationAdd)){
                            $data['ff_fuel_address'] = $fuelOtherStationAdd;
                        }
                        $data['ff_rate'] = $ffRate;
                        $data['ff_fuel_quantity'] = $fuelQuan;
                        $data['ff_amount'] = $fuelAmt;
                        $data['ff_payment_mode'] = $paymentMode;
                        $data['ff_standard_remark'] = $ffTypeRemark;
                        $data['fuel_filling_case'] = $fuelFillingCase;
                        $data['ff_case_type_remark'] = $ffCaseTypeRemark;
                        if($fuelFillingCase == 'fuel_filling_during_case'){
                            $data['amb_fuel_status'] = 'Available';
                            $data['is_updated'] = '1';
                        }else{
                            $data['amb_fuel_status'] = 'Standby Fuel Filling';
                        }
                        $data['ff_voucher_no'] = $ffBill;
                        ...
                        if($chkUploadImg == "true") {
                            ...
                            if(sizeof($errors)==0)
                            {
                                ...
                                $data['ff_nozzle_image'] = 'assets/images/nozzle_slip/'.implode(",",$image1);
                            }
                        }
                        // print_r($data);die;
                        ...
                        $summery['off_road_status'] = 'Standby Fuel Filling'; //$SummeryOffRdStatus
                        $summery['off_road_remark'] = $stndRemark;
                        // $summery['off_road_remark_other'] = $otherRemark ? $otherRemark : "";
                        $summery['off_road_date'] = date('Y-m-d', strtotime($fuelDateTime));
                        $summery['off_road_time'] = date('H:i:s', strtotime($fuelDateTime));
                        ...
                        $timestamp['start_odmeter'] = $ffPreviousOdometer;
                        $timestamp['end_odmeter'] = $currentOdometer;
                        $timestamp['total_km'] = $currentOdometer - $ffPreviousOdometer;
                        $timestamp['timestamp'] = date('Y-m-d H:i:s');
                        $timestamp['odometer_type'] = 'fuel filling';
                        $timestamp['remark'] = $stndRemark;
                        // $timestamp['other_remark'] = $otherRemark ? $otherRemark : "";
                        $timestamp['remark_type'] = 'fuel_filling';
                        if($fuelFillingCase == 'fuel_filling_during_case'){

                            $timestamp['odometer_type'] = 'fuel_filling_during_case';
                        }
    ...
    public function fuelStation_post(){
        if((isset($_COOKIE['cookie']))){
            ...
            $fuelDistId = $this->post('districtId');
            // echo $deviceId;
            $deviceIdLogindata = $this->user->getAmbDist($deviceId);
            $dis = array();
            if(!empty($deviceIdLogindata)){
                foreach($deviceIdLogindata as $deviceIdLogindata1){
                    array_push($dis,$deviceIdLogindata1['amb_district']);
                    // die;
                }
            }
            // $station = $this->user->FuelStation($dis[0]);
            $station = $this->user->FuelStation($fuelDistId);
            $data1 = array();

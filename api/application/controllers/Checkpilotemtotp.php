...
class Checkpilotemtotp extends REST_Controller {
                    ...
                    $pilotLoginCheck = $this->user->checkBoth1($checklogin);
                        ...
                        if($otpExpireTime[0]['clg_password'] != $password){
                            ...
                            if($otpExpireTime[0]['clg_password'] == $password){
                                ...
                                $this->user->logoutanotherdevicelogin($checklogin);
                        ...
                        if($chkAnotherDeviceLogin == 1){
                            ...
                            if(!empty($chkPilotUserLogin) && !empty($chkEmtUserLogin)){
                                ...
                                $this->response([
                                    ...
                                    'error' => ([
                                        'code' => 1,
                                        'message' => 'The users are already logged into this ambulance('.$chkEmtLogin[0]['vehicle_no'].')'
                                    ])
                                ],REST_Controller::HTTP_OK);
                ...
                if(!empty($typeId) && !empty($pilotId) && !empty($emtId)){
                    ...
                    if($chkSameDeviceLogin == 1){
                        if(($emtOtp[0]['otp_expire_time'] >= $current_time) && ($pilotOtp[0]['otp_expire_time'] >= $current_time)){
                            if(($emtOtp[0]['otp'] == $emtotp) && ($pilotOtp[0]['otp'] == $otp)){
                                $data = array(
                                    'otp' => '',
                                    'otp_expire_time' => '',
                                    'otp_verification' => 2
                            ...
                            if(($pilotOtp[0]['otp_expire_time'] >= $current_time)){
                                $user = 'PILOT';
                            }else if(($emtOtp[0]['otp_expire_time'] >= $current_time)){
                                $user = 'EMSO';
                            }else{
                                $user = 'Both';
                            }
                    ...
                    $this->response([
                        ...
                        'error' => ([
                            'code'=>10,
                            'message' => 'Please Pass Pilot and EMT Mobile No'
                        ])
                    ],REST_Controller::HTTP_OK);
                }
            }
    ...
    public function getattendacewise_post(){
        ...
        $vehicle = $this->post('vehicleNumber');
        $veh = explode(' ',$vehicle);
        $vehicalNumber = implode('-',$veh);
        // echo $vehicalNumber;die;
        $current_date = date('Y-m-d H:i:s');  
        $id = $this->encryption->decrypt($_COOKIE['cookie']);

...
class User extends CI_Model {
    ...
    public function getVehicleData($vehicleNumber){
     $data="SELECT *,ward.ward_name as wname FROM ems_ambulance as amb LEFT JOIN ems_mas_ward as ward ON ward.ward_id=amb.ward_name LEFT JOIN ems_hospital as base ON base.hp_id =amb.amb_base_location WHERE amb.amb_rto_register_no= '".$vehicleNumber."' AND amb.ambis_deleted = '0' ";
    ...
    public function assignedIncidenceCalls($ambno){
        $data = "SELECT incamb.*,inc.*,cls.*,clr.*,rel.*,op.*,mn.*,pc.*,hosp.*" 
                ...
                . "LEFT JOIN $this->micnature as mn on(mn.ntr_id=inc.inc_mci_nature) "
                . "LEFT JOIN $this->patientComplaint as pc on(pc.ct_id=inc.inc_complaint) "
                . "LEFT JOIN ems_hospital1 as hosp on(inc.hospital_id=hosp.hp_id) "
    ...
    public function getPatientMedInfo($patientId){
        $data = "SELECT epcr.*,ptn.*,dst.*,thl.*,city.*,loc.*,proimp.*,ptnres.reason,casetype.case_name "
    ...
    public function EditPatientHandoverCondition($patientId){
        $hand = "SELECT epcr.*,imp.*,stats.id,stats.name,loc.*,casetype.case_id,casetype.case_name FROM `ems_epcr` as epcr "
 ...
 //echo $this->db->last_query;
    ...
    public function getendodometer($ambulanceno){
        $sql = "SELECT amb_time.*,max(end_odmeter*1) as end_odmeter FROM $this->closure as amb_time Where amb_rto_register_no = '".$ambulanceno."' AND flag = '1' AND end_odmeter != '' ORDER BY amb_time.end_odmeter DESC,amb_time.timestamp DESC Limit 0, 1";
    ...
    public function checkPtnComToTheHosp($incidentId){
        ...
        $data = "SELECT ei.patient_ava_or_not, ei.inc_ref_id, ei.inc_duplicate,ei.incis_deleted, ee.pat_com_to_hosp FROM ems_incidence AS ei "
                ...
                ."WHERE ei.patient_ava_or_not = 'yes' AND ei.inc_duplicate='no' AND ei.incis_deleted = '0' AND ei.inc_ref_id = '$incidentId' ";
        return $this->db->query($data)->result_array();
    }
...
function getallambulancesstatus($ambno){
    ...
    public function chkPtnAva($incidentId){
        return $this->db->where('inc_id',$incidentId)->get('ems_incidence_patient')->result_array();
    }
    ...
    public function getvehicledata1($data){
        $data="SELECT amb.amb_rto_register_no,hosp.hp_name,st.ambs_name,ambtype.ambt_name,dis.dst_name
            FROM ems_ambulance as amb
            LEFT JOIN ems_hospital as hosp ON amb.amb_base_location=hosp.hp_id

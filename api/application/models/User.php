...
class User extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->feedback = 'ems_app_feedback';
    ...
    public function getVehicleData($vehicleNumber){
     $data="SELECT *,ward.ward_name as wname FROM ems_ambulance as amb "
        ."LEFT JOIN ems_mas_ward as ward ON ward.ward_id=amb.ward_name "
        ."LEFT JOIN ems_base_location as base ON base.hp_id =amb.amb_base_location "
        ."WHERE amb.amb_rto_register_no= '".$vehicleNumber."' AND amb.ambis_deleted = '0' ";
    //  var_dump($data);die;
    ...
    public function getCompIncidence($ambno,$loginId){
        $data = "SELECT incamb.*,inc.*,cls.*,clr.*,rel.*,op.*,mn.*,pc.*,pur.pname " 
       ...
       //  print_r($data);die;
        ...
        foreach($compIncidentRec as $record){
            ...
            $recordData1['callType'] = $record['pname'];
    ...
    public function assignedIncidenceCalls($ambno){
        $data = "SELECT incamb.*,inc.*,cls.*,clr.*,rel.*,op.*,mn.*,pc.*,hosp.*,pur.pname,pur.pcode " 
                ."FROM $this->incidenceamb AS incamb "
                ."LEFT JOIN $this->incidence AS inc ON (inc.inc_ref_id = incamb.inc_ref_id) "
                ."LEFT JOIN ems_mas_call_purpose AS pur ON (inc.inc_type = pur.pcode) "
                . "LEFT JOIN $this->calls AS cls ON (cls.cl_id = inc.inc_cl_id) "
        ...
        foreach($compIncidentRec as $record){
            ...
            if($record['inc_type'] == 'on_scene_care'){
                $recordData1['onsceneCare'] = 2;
            }else{
                if($record['inc_type'] == 'MCI'){
                    $recordData1['cheifComplaint'] = $record['ntr_nature'];
                }else{
    ...
    public function logout($id,$type){
        ...
        foreach($data as $data1){
            ...
            file_put_contents('logs/Query_log/loginSession_logout_query_log-' . date('Y-m-d') . '.php', $sql.",\r\n", FILE_APPEND);
        }
        return 1;
    }
    ...
    public function addPatient($data){
        ...
        $this->db->select('ptn_id');
        $this->db->from('ems_patient');
        $this->db->where('p_id',$last_insert_id);
    ...
    public function getPatientInfo($PatientId){
        $this->db->select('ptn.*,blood.bldgrp_id,blood.bldgrp_name');
        $this->db->from('ems_patient as ptn');
        $this->db->join('ems_mas_bloodgrp_type as blood','ptn.ptn_bgroup=blood.bldgrp_id','left');
    ...
    public function getPatientDetails($incidentId){
        ...
        foreach($incPtnId as $incPtnId1){
            ...
            $id = $patient['id'];
            $data1 = "SELECT ptn.*,epcr.initial_complete,epcr.ongoing_complete,epcr.handover_cond_complete,epcr.handover_issue_complete,epcr.medicine_complete,epcr.consumable_complete,epcr.pat_com_to_hosp,epcr.pat_com_to_hosp,epcr.pat_com_to_hosp_reason,epcr.pat_com_to_hosp_reason_other,ptnreason.id,ptnreason.reason,blood.bldgrp_id,blood.bldgrp_name "
                    ."FROM `ems_patient` AS ptn "
                    ."LEFT JOIN `ems_mas_bloodgrp_type` As blood ON (ptn.ptn_bgroup = blood.bldgrp_id) "
            ...
            foreach($data as $ptn1){
                ...
                if($ptn1['ongoing_complete'] == '0'){
                    $ongoing_complete = 1;  //$ongoing_complete = 0;
                }else if($ptn1['ongoing_complete'] == '1'){
                    $ongoing_complete = 1;
                }else{
                    $ongoing_complete = 1;  //$ongoing_complete = -1;
                }
                ...
                $patientDetails = array(
                    ...
                    'bloodGroup' => ([
                        'id' => isset($ptn1['bldgrp_id']) ? (int) $ptn1['bldgrp_id'] : (int) 0,
                        'name' => isset($ptn1['bldgrp_name']) ? $ptn1['bldgrp_name'] : "",
                    ]),
    ...
    public function getIncidentData($incidentID){
        $data = "SELECT inc.*,pur.pname ,incamb.amb_rto_register_no,hosp.hp_name,hospType.full_name,ptnComp.ct_type,incptn.ptn_id,ptn.ayushman_id "
                ."FROM `ems_incidence` As inc "
                ."LEFT JOIN `ems_mas_patient_complaint_types` As ptnComp On (inc.inc_complaint = ptnComp.ct_id) "
                ."LEFT JOIN `ems_mas_call_purpose` As pur On (inc.inc_type = pur.pcode) "
                ."LEFT JOIN `ems_incidence_ambulance` As incamb On (inc.inc_ref_id = incamb.inc_ref_id) "
                ."LEFT JOIN `ems_hospital` As hosp On (inc.hospital_id = hosp.hp_id) "
                ...
                ."LEFT JOIN `ems_patient` As ptn On (incptn.inc_id = ptn.ptn_id) "
                ."WHERE inc.inc_ref_id = '$incidentID' ";
    ...
    public function getPatientMedInfo($patientId){
        $data = "SELECT epcr.*,ptn.*,dst.*,thl.*,city.*,loc.*,proimp.*,ptnres.reason,casetype.case_name,blood.bldgrp_id,blood.bldgrp_name "
                ...
                ."LEFT JOIN `ems_mas_provider_casetype` AS casetype ON (epcr.provider_casetype=casetype.case_id)"
    ...
    public function addPatientInitialAssessment($patientId,$data){
        ...
        $this->db->select('id,inc_ref_id,amb_reg_id,base_month');
        $this->db->where("ptn_id", "$patientId");
        $this->db->from('ems_epcr');
        $fetched = $this->db->get();
        $result = $fetched->num_rows();
        //echo $result;
        if($result != '0'){
    ...
    public function EditPatientInitialAssessment($patientId){
	    ...
	    if($initial1['ercp_advice'] == 'advice_Yes'){
                $adviceTaken = 'yes';
            }else if($initial1['ercp_advice'] == 'advice_No'){
                $adviceTaken = 'no';
            }
            $medicaladvicegivenby = $this->getmedicaladvicegivenby($initial1['ercp_advice_Taken']);
            $pupilsleft = $this->getpupilsasperidleft($initial1['ini_con_pupils']);
            $pupilsright = $this->getpupilsasperidright($initial1['ini_con_pupils_right']);
            $injury = json_decode($initial1['ini_con_injury']);
        }
        ...
        if(!empty($initial)){
		if(isset($initial[0]['ini_con_gcs'])){
	    		$cgs['id'] = (int) $initial[0]['ini_con_gcs'];
            		$cgs['name'] =  (string) $initial[0]['ini_con_gcs'];
		}else{
			$cgs['id'] = (int) $initial[0]['ini_con_gcs'];
            		$cgs['name'] =  Null;
		}
            $iniedit = array(
                'id' => (string) $initial[0]['ptn_id'],
                'loc' => $loc,
                'iniAirway' => $initial[0]['ini_airway'],
                'iniBreathing' => isset($initial[0]['ini_breathing']) ? $initial[0]['ini_breathing'] : '',
                ...
                'iniConPupilsLeft' => isset($pupilsleft) ? $pupilsleft : Null,
                'iniConPupilsRight' => isset($pupilsright) ? $pupilsright : Null,
                'iniInjury' => isset($injury) ? $injury : [],
                "iniConBsl" => isset($initial[0]['ini_con_bsl']) ? $initial[0]['ini_con_bsl'] : Null,
                "iniConGcs" => $cgs ? $cgs : null,
                "iniMedAdGiven" => isset($adviceTaken) ? $adviceTaken : "",
                "iniAdGivenBy" => isset($medicaladvicegivenby) ? $medicaladvicegivenby : '',
		        'iniCirRadial' => isset($initial[0]['ini_con_circulation_radial']) ? $initial[0]['ini_con_circulation_radial'] : '',
                'iniCirCarotid' => isset($initial[0]['ini_con_circulation_carotid']) ? $initial[0]['ini_con_circulation_carotid'] : '',
                'iniCirBrachial' => isset($initial[0]['ini_con_circulation_brachial']) ? $initial[0]['ini_con_circulation_brachial'] : '',
		'iniRR' => $initial[0]['ini_con_rr'] ? (int) $initial[0]['ini_con_rr'] : null
            );
    ...
    public function EditPatientHandoverCondition($patientId){
        $hand = "SELECT epcr.*,imp.*,stats.id,stats.name,loc.*,casetype.case_id,casetype.case_name,deliamb.deliv_in_amb_take_photo FROM `ems_epcr` as epcr "
                ...
                ."LEFT JOIN `ems_patient_status` as stats On (epcr.hc_ps = stats.id) "
                ."LEFT JOIN `ems_mas_delivery_in_amb` as deliamb On (epcr.inc_ref_id = deliamb.deliv_in_amb_inc_ref_id) WHERE `ptn_id` = $patientId ";
        $handover = $this->db->query($hand)->result_array();
	foreach($handover as $handover1){
            
	    if(empty($handover1['hc_con_pupils'])){
                $handover['pupilsleft'] = $this->getpupilsasperidleft($handover1['ini_con_pupils']);
            }else{
                $handover['pupilsleft'] = $this->getpupilsasperidleft($handover1['hc_con_pupils']);
            }
            if(empty($handover1['hc_con_pupils_right'])){
                $handover['pupilsright'] = $this->getpupilsasperidright($handover1['ini_con_pupils_right']);
            }else{
                $handover['pupilsright'] = $this->getpupilsasperidright($handover1['hc_con_pupils_right']);
            }
        }
        // $data = array();
        // foreach($handover as $handover1){
    ...
    //     return $cons;
    // }
    public function ConsumableList($id,$args,$ambulanceNo){
        $ambType = $this->db->where('amb_rto_register_no',$ambulanceNo)->get('ems_ambulance')->result_array();
        $consumable = $this->db->where('inv_type','CA')->where('invis_deleted','0')->like('inv_usedby_amb_type',$ambType[0]['amb_type'])->get($this->inventory)->result_array();
        $a1 = array();
        foreach($consumable as $consumable1){
            $a = array(
                'id' => (int) $consumable1['inv_id'],
        ...
        if($args == 'CA'){
            ...
            $ambType = $ambType[0]['amb_type'];
            $consumable = "SELECT * "
                        . "FROM $this->indentItem as item "
                        . "LEFT JOIN $this->inventory as inv ON (item.ind_item_id = inv.inv_id) "
                        . "WHERE item.ind_req_id = $id AND item.ind_item_type = 'CA' AND item.indis_deleted = '0' AND inv.`inv_usedby_amb_type` LIKE '%$ambType%' ";
    ...
    public function ConsumableNonUnitList($id,$args,$ambulanceNo){
        $ambType = $this->db->where('amb_rto_register_no',$ambulanceNo)->get('ems_ambulance')->result_array();
        $consumable = $this->db->where('inv_type','NCA')->where('invis_deleted','0')->like('inv_usedby_amb_type',$ambType[0]['amb_type'])->get($this->inventory)->result_array();
        $a1 = array();
        foreach($consumable as $consumable1){
            $a = array(
                'id' => (int) $consumable1['inv_id'],
        ...
        if($args == 'CA'){
            ...
            $consumable = "SELECT * "
                        . "FROM $this->indentItem as item "
                        . "LEFT JOIN $this->inventory as inv ON (item.ind_item_id = inv.inv_id) "
                        . "WHERE item.ind_req_id = $id AND item.ind_item_type = 'NCA' AND item.indis_deleted = '0' AND inv.`inv_usedby_amb_type` LIKE '%$ambType%' ";
            $consumable1 = $this->db->query($consumable)->result_array();
            $list = array();
            foreach($consumable1 as $con){
                $data = array(
    ...
    public function EquipmentList($id,$args){
        // ->where('eqp_type','amb')
        $consumable = $this->db->where('eqp_type','1')->where('eqpis_deleted','0')->get($this->equipment)->result_array();
        $a1 = array();
        foreach($consumable as $consumable1){
            $a = array(
        ...
        if($args == 'EQP'){
            ...
            // $consumable = "SELECT * "
            //             . "FROM $this->equipment as eqp "
            //             . "WHERE eqp.eqp_status = '1' AND eqp.eqpis_deleted = '0' ";
            // $consumable1 = $this->db->query($consumable)->result_array();
            // $list = array();
            foreach($consumable1 as $con){
                $data = array(
    ...
    //     // return $medicines;
    // }
     public function getmedicinename($id,$args,$ambulanceNo){
        $ambType = $this->db->where('amb_rto_register_no',$ambulanceNo)->get('ems_ambulance')->result_array();
        $ambMedicine = $this->db->where('med_type','amb')->where('med_unit',"MCA")->where('medis_deleted','0')->like('med_usedby_amb_type',$ambType[0]['amb_type'])->get($this->invMedicine)->result_array();
        $a1 = array();
        foreach($ambMedicine as $ambMedicine1){
            $a = array(
                'id' => (int) $ambMedicine1['med_id'],
        ...
        if($args == 'MCA'){
            ...
            $medicine = "SELECT * "
                        ...
                        . "WHERE item.ind_req_id = $id AND item.ind_item_type = 'MCA' AND item.indis_deleted = '0' AND med.`med_usedby_amb_type` LIKE '%$ambType%' ";
    ...
    //     return $cons;
    // }
    public function getNonUnitMedicineName($id,$args,$ambulanceNo){
        $ambType = $this->db->where('amb_rto_register_no',$ambulanceNo)->get('ems_ambulance')->result_array();
        $ambMedicine = $this->db->where('med_type','amb')->where('med_unit','MNCA')->where('medis_deleted','0')->like('med_usedby_amb_type',$ambType[0]['amb_type'])->get($this->invMedicine)->result_array();
        $a1 = array();
        foreach($ambMedicine as $ambMedicine1){
            $a = array(
                'id' => (int) $ambMedicine1['med_id'],
        ...
        if($args == 'MNCA'){
            ...
            $medicine = "SELECT * "
                        . "FROM $this->indentItem as item "
                        . "LEFT JOIN $this->invMedicine as med ON (item.ind_item_id = med.med_id) "
                        . "WHERE item.ind_req_id = $id AND item.ind_item_type = 'MNCA' AND item.indis_deleted = '0' AND med.`med_usedby_amb_type` LIKE '%$ambType%' ";
            $medicine1 = $this->db->query($medicine)->result_array();
            $list = array();
            foreach($medicine1 as $con){
                $data = array(
    ...
    public function checkAllInfo_old($incidentID){
        $incPtnId = $this->db->where('inc_id',$incidentID)->get($this->incidencePatient)->result_array();
        $ptn = array();
        $pt1 = array(); 
        foreach($incPtnId as $incPtnId1){
            ...
            $data1 = "SELECT ptn.*,epcr.initial_complete,epcr.ongoing_complete,epcr.handover_cond_complete,epcr.handover_issue_complete,epcr.medicine_complete,epcr.consumable_complete,epcr.pat_com_to_hosp,epcr.pat_com_to_hosp,epcr.pat_com_to_hosp_reason,ptnreason.id,ptnreason.reason "
                    ...
                    ."LEFT JOIN `ems_epcr` AS epcr ON (ptn.ptn_id = epcr.ptn_id) "
                    ."LEFT JOIN `ems_patient_notcomtohosp_reason` AS ptnreason ON (epcr.pat_com_to_hosp_reason = ptnreason.id) "
                    ."WHERE ptn.ptn_id = '$id' AND ((epcr.pat_com_to_hosp == '0') OR (epcr.pat_com_to_hosp == '')) AND ((epcr.initial_complete == '0' OR epcr.initial_complete == NULL) OR (epcr.ongoing_complete == '0' OR epcr.ongoing_complete == NULL) OR (epcr.handover_cond_complete == '0' OR epcr.handover_cond_complete == NULL) OR (epcr.handover_issue_complete == '0' OR epcr.handover_issue_complete == null) ) ";
            $data = $this->db->query($data1)->result_array();
                array_push($pt1,$data);
        }
        return $pt1;
    ...
    public function checkAllInfo($incidentID){
        // return $this->db->where('inc_ref_id',$incidentID)->get($this->epcr)->result_array();
		$data = "SELECT * FROM ems_incidence_patient WHERE inc_id = '".$incidentID."' ";
		  $incPtnId = $this->db->query($data)->result_array();
 ...
 //echo $this->db->last_query;
    ...
    public function getendodometer($ambulanceno){
        $sql = "SELECT amb_time.id,amb_time.amb_rto_register_no,amb_time.inc_ref_id,max(end_odmeter*1) as end_odmeter FROM $this->closure as amb_time "
                ."Where amb_rto_register_no = '".$ambulanceno."' AND flag = '1' AND end_odmeter != '' "
                . "AND amb_time.odometer_type IN ('closure','Chng_new_fitted_odometer','update tyre maintaince','tyre maintenance','update onroad offroad maintaince','onroad_offroad maintenance','update accidental maintaince','accidental maintenance','Scrape Vahicle Update','Scrape Vahicle','update preventive maintenance','preventive maintenance','Manpower Update','Manpower','update breakdown maintaince','breakdown maintenance') "
                ."ORDER BY amb_time.end_odmeter DESC,amb_time.timestamp DESC Limit 0, 1";
                // print_r($sql);die;
        $result = $this->db->query($sql);
        if ($result) {
            $end = $result->result_array();
            if(empty($end[0]['end_odmeter'])){
                $odometers = array(
                    'id' => 0,
                    'endOdometer' => 0
                );
        ...
        // $this->db->select('id,amb_rto_register_no,end_odmeter,flag');
        // $this->db->select_max('end_odmeter', 'max');
        // $this->db->from($this->closure);
        // $this->db->where('amb_rto_register_no',$ambulanceno);
        // $this->db->where('end_odmeter != ','');
        // $this->db->where('flag','1');
    ...
    public function getHospitalDeatils($incidentId){
         $data = "SELECT inc.*,hosp.*,dist.dst_code as id,dist.dst_name as name FROM `ems_incidence` as inc"
                ." LEFT JOIN `ems_hospital` as hosp ON (inc.hospital_id = hosp.hp_id) "
                ." LEFT JOIN `ems_mas_districts` as dist ON (hosp.hp_district = dist.dst_code)"
    ...
    public function getDistrict($incidentId){
        $data = "SELECT dis.dst_code,dst_name FROM `ems_incidence`"
                ...
                ." WHERE inc.inc_ref_id = '".$incidentId."' AND dis.dstis_deleted = '0' ORDER BY dis.dst_name ASC ";
        return $this->db->query($data)->result_array();
    }
    ...
    public function updateLoginSession($data1,$data){
        if($data['typeId'] == 3){
            ...
            $sql = $this->db->last_query() . " \n time:'".date('Y-m-d H:i:s') ; 
            file_put_contents('logs/Query_log/updateLoginSession_query_log-' . date('Y-m-d') . '.php', $sql.",\r\n", FILE_APPEND);
     ...
     public function chkAnotherDeviceLogin($data){
        ...
        $data1['log_remark'] = 'Logined from another device';
        if($data['typeId'] == 1){         
            ...
            if((count($chk)==1) && ($chk[0]['device_id'] != $data['deviceid']) && ($chk[0]['vehicle_no'] == $data['vehicleNumber'])){
                $this->db->where('status','1')->where('clg_id',$data['pilotId'])->update($this->loginSessionTbl,$data1);
                $sql = $this->db->last_query() . " \n time:'".date('Y-m-d H:i:s') ; 
                file_put_contents('logs/Query_log/chkAnotherDeviceLogin_query_log-' . date('Y-m-d') . '.php', $sql.",\r\n", FILE_APPEND);
                return 1;
            }else{
                return 2;
            }
            ...
            if((count($chk)==1) && ($chk[0]['device_id'] != $data['deviceid']) && ($chk[0]['vehicle_no'] == $data['vehicleNumber'])){
                $this->db->where('status','1')->where('clg_id',$data['emtId'])->update($this->loginSessionTbl,$data1);
                $sql = $this->db->last_query() . " \n time:'".date('Y-m-d H:i:s') ; 
                file_put_contents('logs/Query_log/chkAnotherDeviceLogin_query_log-' . date('Y-m-d') . '.php', $sql.",\r\n", FILE_APPEND);
                return 1;
            }else{
            ...
            if((count($chk)==1) && ($chk[0]['device_id'] != $data['deviceid'])){
                $this->db->where('status','1')->where('clg_id',$data['clg_id'])->update($this->loginSessionTbl,$data1);
                $sql = $this->db->last_query() . " \n time:'".date('Y-m-d H:i:s') ; 
                file_put_contents('logs/Query_log/chkAnotherDeviceLogin_query_log-' . date('Y-m-d') . '.php', $sql.",\r\n", FILE_APPEND);
                return 1;
            }else{
    ...
    public function incidentCallsDetails($incidentId){
        //$chkonscene = $this->db->where('inc_ref_id',$incidentId)->get('ems_incidence')->result_array();
		  $data = "SELECT * FROM ems_incidence WHERE inc_ref_id = '".$incidentId."' ";
		  $chkonscene = $this->db->query($data)->result_array();
        if(!empty($chkonscene)){
            if($chkonscene[0]['inc_type'] == "on_scene_care"){
                $data = "SELECT inc.*,cls.cl_id,cls.cl_clr_id,cls.clr_ralation,clr.* " 
                ."From $this->incidence AS inc "
                . "LEFT JOIN $this->calls AS cls ON (cls.cl_id = inc.inc_cl_id) "
                . "LEFT JOIN $this->callers AS clr ON (clr.clr_id = cls.cl_clr_id) "
                . "WHERE inc.inc_ref_id = '".$incidentId."' ";
                $compIncidentRec = $this->db->query($data)->result_array();
                ...
                foreach($compIncidentRec as $record){
                    ...
                    $recordData1['incidentPincode'] = (float) $record['inc_pincode'];
                    // $recordData1['CallerRelationName'] = $record['rel_name'];
                    $recordData1['CallerRelationName'] = null;
                    // $recordData1['Services'] = $services;
                    $recordData1['Services'] = "Medical";
                    //$recordData1['cheifComplaint'] = 'on_scene_care';
                    $recordData1['hospitalFlag'] = 'false';
                    $recordData1['hospitalName'] = null;
                    $recordData1['hospitalAddress'] = null;
                    $recordData1['hospitalLat'] = null;
                ...
                $data = "SELECT incamb.amb_rto_register_no,incamb.inc_ref_id,incamb.inc_amb_id,inc.*,cls.cl_id,cls.cl_clr_id,cls.clr_ralation,clr.*,mn.ntr_id,mn.ntr_nature,mn.ntr_services,pc.ct_id,pc.ct_type,hosp.hp_id,hosp.hp_name,hosp.hp_address,hosp.hp_lat,hosp.hp_long,pvthosp.pr_total_amount,pcrform.pcrform_image,purcall.pcode,dropback.home_address " 
                ."FROM $this->incidenceamb AS incamb "
                ."LEFT JOIN $this->incidence AS inc ON (inc.inc_ref_id = incamb.inc_ref_id) "
                . "LEFT JOIN $this->calls AS cls ON (cls.cl_id = inc.inc_cl_id) "
                . "LEFT JOIN $this->callers AS clr ON (clr.clr_id = cls.cl_clr_id) "
                ...
                . "LEFT JOIN ems_private_hospital as pvthosp on(inc.inc_ref_id=pvthosp.pr_inc_ref_id) "
                . "LEFT JOIN ems_app_pcrform_upload as pcrform on(inc.inc_ref_id=pcrform.pcrform_inc_ref_id) "
                . "LEFT JOIN ems_back_drop_call as dropback on(inc.inc_ref_id=dropback.inc_ref_id) "
                ...
                foreach($compIncidentRec as $record){
                    ...
                    if(empty($record['pcrform_image'])){
                        $recordData1['pcrformupload'] = false;
                    }else{
                        $recordData1['pcrformupload'] = true;
                    }
                    $recordData1['pr_total_amount'] = $record['pr_total_amount'];
                    $recordData1['pcode'] = $record['pcode'];
                    $recordData1['dropBackHomeAddress'] = $record['home_address'];
                    array_push($recordData,$recordData1);
                }
            }
        }
    ...
    public function incidentCallsDetails_old($incidentId){
        $data = "SELECT incamb.amb_rto_register_no,incamb.inc_ref_id,incamb.inc_amb_id,inc.*,cls.cl_id,cls.cl_clr_id,cls.clr_ralation,clr.*,mn.ntr_id,mn.ntr_nature,mn.ntr_services,pc.ct_id,pc.ct_type,hosp.hp_id,hosp.hp_name,hosp.hp_address,hosp.hp_lat,hosp.hp_long " 
                ."FROM $this->incidenceamb AS incamb "
                ."LEFT JOIN $this->incidence AS inc ON (inc.inc_ref_id = incamb.inc_ref_id) "
                . "LEFT JOIN $this->calls AS cls ON (cls.cl_id = inc.inc_cl_id) "
                . "LEFT JOIN $this->callers AS clr ON (clr.clr_id = cls.cl_clr_id) "
    ...
    public function checkPtnComToTheHosp($incidentId){
        ...
        $data = "SELECT ei.patient_ava_or_not, ei.inc_ref_id, ei.inc_duplicate,ei.incis_deleted, ee.pat_com_to_hosp FROM ems_incidence AS ei "
                ."LEFT JOIN `ems_epcr` AS ee ON ei.inc_ref_id = ee.inc_ref_id "
                ."WHERE ei.patient_ava_or_not = 'yes' AND ei.inc_duplicate='no' AND ei.incis_deleted = '0' AND ei.inc_ref_id = '$incidentId' AND ei.inc_set_amb ='1' AND ee.epcris_deleted='0' ";
        return $this->db->query($data)->result_array();
    }
    ...
    public function chkAmbLogin($data){
        $amb = "SELECT loginSession.*,coll.clg_first_name,coll.clg_mid_name,coll.clg_last_name,coll.clg_mobile_no FROM `ems_app_login_session` AS loginSession " 
                    ."LEFT JOIN ems_colleague as coll ON (loginSession.clg_id = coll.clg_id) "
    ...
    public function chkpilotLogin($data){
        $pilot = "SELECT loginSession.*,coll.clg_first_name,coll.clg_mid_name,coll.clg_last_name,coll.clg_mobile_no FROM `ems_app_login_session` AS loginSession " 
                    ."LEFT JOIN ems_colleague as coll ON (loginSession.clg_id = coll.clg_id) "
                    ."WHERE loginSession.clg_id = '".$data['pilotId']."' AND loginSession.status = '1' ";
        return  $this->db->query($pilot)->result_array(); 
    }
    ...
    public function getfuelstation($fuelStation){
        $data = "SELECT station.f_google_address,station.f_station_mobile_no,station.f_address"
    ...
    public function insertfuelfilling($data){
        ...
        //return 1;
        $this->db->select('*');
        $this->db->from(' ems_fleet_fuel_filling');
        $this->db->where(" ems_fleet_fuel_filling.ff_gen_id", $data['ff_gen_id']);
        $fetched = $this->db->get();
        $present = $fetched->result();
       
        if (count($present) == 0) {

            $result =$this->db->insert(' ems_fleet_fuel_filling', $data);
            if ($result) {
                return $this->db->insert_id();
            } else {
                return false;
    ...
    public function FuelStation($dis){
        $this->db->select('*');
        $this->db->from($this->fuelstation);
        $this->db->where('f_district_code',"$dis");
        $this->db->where('f_is_deleted','0');
        $this->db->order_by("f_station_name","asc");
        return  $this->db->get()->result_array();
...
public function checklogindetails(){
 ...
 public function updateusertOtp($userdata,$data){    
   ...
   public function getoxygenfillingDetails($requestId){       
    ...
    {
        $data1 = $this->db->where('clg_ref_id',$userName)->where('clg_is_active','1')->where('clg_is_deleted','0')->get($this->colleague)->result_array();
    ...
    public function getinventoryname($inventoryType,$Ambulanceno){
        if($inventoryType == 'MCA' || $inventoryType == 'MNCA'){
            ...
            $stk = "SELECT *, "
                    ...
                    ."WHERE med.med_unit = '".$inventoryType."' GROUP BY med.med_id";
...
function getallambulancesstatus($ambno){
    ...
    public function getemtdetails($mobilebNo){
        $this->db->select('*');
        $this->db->from($this->colleague);
        $this->db->where('clg_mobile_no',$mobilebNo);
    ...
    public function getIncdateTime($incidentId){
        $this->db->select('inc_datetime');
        $this->db->from('ems_incidence');
        $this->db->where('inc_ref_id',$incidentId);
        return $this->db->get()->row()->inc_datetime;
    }
    public function getCollId($userName){
        return $this->db->where('clg_ref_id',$userName)->get('ems_colleague')->result_array();
    }
    public function chkPtnAva($incidentId){
		
		$data = "SELECT * FROM ems_incidence WHERE inc_ref_id = '".$incidentId."' ";
		  $data1 = $this->db->query($data)->result_array();
		  return $data1;
		
       // return $this->db->where_in('inc_id',"'".$incidentId."'")->get('ems_incidence_patient')->result_array();
	
    }
    public function getIncRec($incidentId){
        return $this->db->where('inc_ref_id',$incidentId)->get('ems_incidence')->result_array();
    }
    public function getpilotdata($data){
        $this->db->select('clg_id,clg_first_name,clg_mid_name,clg_last_name,clg_mobile_no');
        $this->db->from('ems_colleague');
        $this->db->where('clg_id',$data);
        $data1 = $this->db->get()->result_array();
        return $data1;
    }
    public function getemtdata($data){
        $this->db->select('clg_id,clg_first_name,clg_mid_name,clg_last_name,clg_mobile_no');
        $this->db->from('ems_colleague');
        $this->db->where('clg_id',$data);
        $data1 = $this->db->get()->result_array();
    ...
    public function getvehicledata1($data){
        $data="SELECT amb.amb_rto_register_no,baseloc.hp_name,st.ambs_name,ambtype.ambt_name,dis.dst_name
            FROM ems_ambulance as amb
            LEFT JOIN ems_base_location as baseloc ON amb.amb_base_location=baseloc.hp_id
            LEFT JOIN ems_mas_ambulance_status as st ON amb.amb_status=st.ambs_id
            LEFT JOIN ems_mas_ambulance_type as ambtype ON amb.amb_type=ambtype.ambt_id
            LEFT JOIN ems_mas_districts as dis ON amb.amb_district=dis.dst_code
            WHERE  amb.amb_rto_register_no = '".$data."' ";
        $data1 =  $this->db->query($data)->result_array();
         //echo $this->db->last_query();die;
        return $data1;
    }
    public function getlatlong($countId1){
        return $this->db->where('id >', $countId1)->get('ems_mas_amb_latlong')->result_array();
    }
    public function insertInjury($injury){
        //foreach($iniInjMatrixArray as $iniInjMatrixArray1){
            $this->db->select('*');
            $this->db->from('ems_ambulance_stock');
            $this->db->where("as_item_id", $injury['as_item_id']);
            $this->db->where("as_item_type", $injury['as_item_type']);
            $this->db->where("as_sub_id", $injury['as_sub_id']);
            $fetched = $this->db->get();
            $present = $fetched->num_rows();
		//print_r($injury);die;
            if($present == '0'){
                $res = $this->db->insert('ems_ambulance_stock', $injury);    
            }else{
                $this->db->where("as_item_id", $injury['as_item_id']);
                $this->db->where("as_item_type", $injury['as_item_type']);
                $this->db->where("as_sub_id", $injury['as_sub_id']);
                $res = $this->db->update('ems_ambulance_stock', $injury);
            }
        //}
    }
    public function getpupilsasperidleft($id){
        $this->db->select('pp_id as id, pp_type as name');
        $this->db->from('ems_mas_pupils_type');
        ...
        if(empty($data)){
            ...
            foreach($data as $data1){
                $pupils['id'] = (int) $data1['id'];
                $pupils['name'] =  $data1['name'];
                return $pupils;
            }
        }
    }
    public function getpupilsasperidright($id){
        $this->db->select('pp_id as id, pp_type as name');
        $this->db->from('ems_mas_pupils_type');
        $this->db->where("pp_id", $id);
        $data = $this->db->get()->result_array();
    ...
    public function getinjuryasperid($id,$inc_ref_id){
        $this->db->select('stk.as_item_id,stk.as_sub_id,stk.as_item_type,stk.as_sub_type,stk.incidentId,inj.inj_id as id, inj.inj_name as name');
        $this->db->from('ems_ambulance_stock as stk');
        $this->db->join('ems_mas_injury as inj','stk.as_item_id = inj.inj_id');
        $this->db->where("stk.as_sub_id", $id);
        $this->db->where("stk.as_item_type", 'INJ');
        $this->db->where("stk.as_sub_type", 'pcr');
        $this->db->where("stk.incidentId", $inc_ref_id);
        $data = $this->db->get()->result_array();
        $inj = array();
        foreach($data as $data1){
            $inj1['id'] = (int) $data1['id'];
            $inj1['name'] =  $data1['name'];
            array_push($inj,$inj1);
        }
        return $inj;
    }
    public function getmedicaladvicegivenby($id){
        $this->db->select('clg_id, clg_first_name');
        $this->db->from('ems_colleague');
        $this->db->where("clg_id", $id);
        $data = $this->db->get()->result_array();
        ...
        if(empty($data)){
            ...
            foreach($data as $data1){
                $givenby['id'] = (int) $data1['clg_id'];
                $givenby['name'] =  $data1['clg_first_name'];
                return $givenby;
            }
        }
    }
    public function getBaseLoc($incidentID){
        $this->db->select('inc_amb_id, base_location_id,base_location_name');
        $this->db->from('ems_incidence_ambulance');
        $this->db->where("inc_ref_id", $incidentID);
        $data = $this->db->get()->result_array();
        return $data;
    }
    public function insertAbandoneRem($abond){
        $this->db->insert('ems_denial_ambulance_remarks',$abond);
    }
    public function getAmbBaseLoc($vehicleNumber){
        $this->db->select('amb.amb_base_location,bsloc.hp_name');
        $this->db->from('ems_ambulance as amb');
        $this->db->join('ems_base_location as bsloc','amb.amb_base_location = bsloc.hp_id');
        $this->db->where("amb.amb_rto_register_no", $vehicleNumber);
        $data = $this->db->get()->result_array();
        return $data;
    }
    public function getAmbDist($deviceId){
        $this->db->select('sess.vehicle_no,sess.device_id,sess.status,amb.amb_district,amb.amb_rto_register_no');
        $this->db->from('ems_app_login_session as sess');
        $this->db->join('ems_ambulance as amb','sess.vehicle_no = amb.amb_rto_register_no');
        $this->db->where("sess.device_id", "$deviceId");
        $this->db->where("sess.status", '1');
        // var_dump();die;
        $data = $this->db->get()->result_array();
        return $data;
    }
    public function getincPtnCount($incidentId){
        $this->db->select('incptn.inc_id,incptn.ptn_id,');
        $this->db->from('ems_incidence_patient as incptn');
        $this->db->join('ems_patient as ptn','incptn.ptn_id = ptn.ptn_id');
        $this->db->where("incptn.inc_id", "$incidentId");
        // var_dump();die;
        $data = $this->db->get()->result_array();
        return $data;
    }
    public function addPrnRec($ptn_id,$ptn){
        $this->db->where('ptn_id',$ptn_id)->update('ems_patient',$ptn);
    }
    public function insertLoginOdometer($loginodo){
        $this->db->insert('ems_app_login_logout_odo',$loginodo);
    }
    public function get_pvt_hosp_amount($incidentId){
        $data = "SELECT inc.inc_id,inc.inc_ref_id,pvthosp.pr_total_amount " 
                ."FROM $this->incidence AS inc "
                . "LEFT JOIN ems_private_hospital as pvthosp on(inc.inc_ref_id=pvthosp.pr_inc_ref_id) "
                . "WHERE inc.inc_ref_id = '".$incidentId."' ";
        $rec = $this->db->query($data)->result_array();
    ...
    public function update_pvt_hosp_pay_mode($incidentId,$pvthospPayModeAdd){
        $this->db->where('pr_inc_ref_id',$incidentId)->upate('ems_private_hospital',$pvthospPayModeAdd);
    }
    public function insertPcrFormdometer($pcrform){
        $this->db->insert('ems_app_pcrform_upload',$pcrform);
        return 1;
    }
    public function updatePtnAvaOrNot($ptnAvaOrNot,$incidentId){
        $this->db->where('inc_ref_id',$incidentId)->update('ems_incidence',$ptnAvaOrNot);
    }
    public function getpcrformuploadstatus($incidentId){
        return $this->db->where('pcrform_inc_ref_id',$incidentId)->get('ems_app_pcrform_upload')->result_array();
    }
    public function insertDelivInAmb($delivinamb){
        $this->db->insert('ems_mas_delivery_in_amb',$delivinamb);
        return 1;
    }
    public function getAmbDistrict($ambulanceNo){
        $data = "SELECT amb_id,amb_rto_register_no,amb_district FROM `ems_ambulance`"
                ." WHERE amb_rto_register_no = '".$ambulanceNo."' ";
        return $this->db->query($data)->result_array();
    }
    public function getClgRefName($loginId){
        $this->db->select('clg_first_name,clg_last_name');
        $this->db->from($this->colleague);
        $this->db->where('clg_id',$loginId);
        return $this->db->get()->result();
    }

...
class Supervisory_Dashboard_model extends CI_Model {
    ...
    public function get_amb_count($args = array()){
        ...
        $data=$this->db->query("select  amb_rto_register_no  from ems_ambulance where ambis_deleted='0' $condition AND amb_status !='11' ");
    ...
    public function get_offroad_preventive_count($args = array()){
        $condition = "";
        $condition .= " AND amb.amb_user_type IN ('108','102')";
        $condition .= " AND amb.amb_status IN (7)";
        $condition .= " AND prev.mt_ambulance_status IN('Approved')";
        $condition .= " AND prev.mt_isdeleted='0'";
        $data= array();
        $data=$this->db->query("select amb.amb_rto_register_no as amb_number from ems_ambulance_preventive_maintaince as prev LEFT JOIN ems_ambulance as amb ON ( amb.amb_rto_register_no = prev.mt_amb_no ) where amb.ambis_deleted='0' $condition "); 
    ...
    public function get_offroad_tyre_count($args = array()){
        ...
        $data= array();
        $data=$this->db->query("select amb.amb_rto_register_no as amb_number from ems_amb_tyre_maintaince as tyre LEFT JOIN ems_ambulance as amb ON ( amb.amb_rto_register_no = tyre.mt_amb_no ) where amb.ambis_deleted='0' $condition ");
        // echo $this->db->last_query()."<br>";
        // die();
        if($data->num_rows()){
            return $data->num_rows();
    ...
    public function get_offroad_accidental_count($args = array()){
        ...
        $data= array();
        $data=$this->db->query("select amb.amb_rto_register_no as amb_number from ems_amb_accidental_maintaince as acc LEFT JOIN ems_ambulance as amb ON ( amb.amb_rto_register_no = acc.mt_amb_no )  where amb.ambis_deleted='0' $condition ");
        // echo $this->db->last_query()."<br>";
        // die();
        if($data->num_rows()){
            return $data->num_rows();
    ...
    public function get_offroad_breakdown_count($args = array()){
        ...
        $data= array();
        $data=$this->db->query("select amb.amb_rto_register_no as amb_number from ems_amb_breakdown_maintaince as brk LEFT JOIN ems_ambulance as amb ON ( amb.amb_rto_register_no = brk.mt_amb_no )  where amb.ambis_deleted='0' $condition ");
        // echo $this->db->last_query()."<br>";
        // die();
        if($data->num_rows()){
            return $data->num_rows();
    ...
    function get_login_ero()
    {
        // die;
        // $sql = "SELECT clg_id	FROM ems_colleague	WHERE clg_group='UG-ERO'  AND clg_is_login='yes';"

        $sql = "SELECT clg_id"
            . " FROM ems_colleague"
            . " WHERE clg_group='UG-ERO'"
            . "  AND clg_is_login='yes'";
    ...
    function get_fuel_filling_data()
    {
        ...
        $sql = "SELECT SUM(ff.ff_fuel_quantity) as fueltotal"
    ...
    public function supv_ero_login_count($args = array()){
        $condition='';
        if($args['from_date'] != '' && $args['to_date'] != '') {
            $from = $args['from_date'];
            $to = $args['to_date'];
    ...
    public function supv_logins_count($args = array()){ 
        $condition='';
    if($args['group'] !='')
    {
